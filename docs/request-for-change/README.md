...
| [rfc-session-address-model.md](rfc-session-address-model.md) | `draft` | Nothing. Verified at `2ff4ce819`: all four phases remain unstarted. The seam is prior art, not a phase — [#1366](https://github.com/kirodotdev/KiroCrew/pull/1366) plus four follow-ups stopped a chat-app conversation being copied into a second session, and `has_dashboard_surface` replaced the name-prefix capability tests, but the identity still encodes origin. Dozens of conversion sites and at least **7** overlapping source/role classifiers remain, `session_map.json` still has no version envelope, surface capability is still one boolean, and an unbound channel tab still starts a second session |
