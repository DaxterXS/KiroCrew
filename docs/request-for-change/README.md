...
| [rfc-agent-sdk-provider-onboarding.md](rfc-agent-sdk-provider-onboarding.md) | `draft` | Nothing, and nothing is proposed to be built by this document itself — it is a decision record. It measures what adding an agent provider costs on main today, prices each remaining PR of [rfc-crew-agent-sdk-boundary.md](rfc-crew-agent-sdk-boundary.md) against that cost, and recommends against continuing the stack on onboarding grounds. The auth seam it proposes is unbuilt: no `agent_sdk/host_auth.py` exists, and the credential floor, the adapter-own-leaf exclusion and the panel's caveat chain are still hand-maintained per-backend literals |
| [rfc-agent-sdk-provider-onboarding.zh-CN.md](rfc-agent-sdk-provider-onboarding.zh-CN.md) | `draft` | Chinese translation of the row above, with the same status. The repo's first translated doc, so it carries a maintenance rule rather than only a disclaimer: it pins the English `revision` it translates in `translates-revision`, and a revision to the English must update the twin in the same commit or delete it. A `translates-revision` behind the English `revision` means the twin is stale, which is why the pin exists |
