...
`detect_lineage(db)` reads **SQLite's own schema catalog** first, and consults the
...
1. **Completed in the 2026-09-07 pickup: independent host-isolation undo stack.**
   Fifteen root host-floor fixtures now use `_floor_monkeypatch`, including shared Kiro
...
| | |
|---|---|
| Worktree | `/path/to/kirocrew-wt-memory-v2-ui` |
| Branch | `feat/memory-v2-ui` |
| Pre-rebase backup tag | `mv2-ui-prerebase-backup` |
| Python | `/path/to/memory-v2-venv/bin/python` (3.12) |
| Node | `export PATH=/path/to/node/bin:$PATH` |

Host-specific paths are shown as placeholders. The recorded results and source
revisions retain their original provenance.
...
```bash
cd /path/to/kirocrew-wt-memory-v2-ui
PY=/path/to/memory-v2-venv/bin/python
...
```bash
cd website && export PATH=/path/to/node/bin:$PATH
...
`<memory-v2-worktree>` (the machine-specific path is omitted).
...
All 900 cap checks (six caps × 50 queries × three modes) passed. The archived
JSON records the former oversized-row skipping check. The live harness now
requires a bounded snippet with its ID and explicit truncation flag; that changed
check needs a fresh run and inherits none of the archived scores. Chinese,
Japanese and Korean keyword-only fact/episode fixtures and forgotten-row
exclusion passed; these are structural checks, not a multilingual benchmark. Reproduce with `python -m
...
kernel process/session attribution or a process-bound proof issued by the trusted
...
Private execution requires membership in `ACP_BACKENDS_PRIVATE_MEMORY_MCP`,
the pre-session registry capability for direct tools inside the member sandbox.
Kiro, Claude Code and KAS are members. The current public Codex ACP backend and
unknown or merely selectable backends are refused before startup. The independent
OS sandbox requirement still applies. Select a supported member backend for private
