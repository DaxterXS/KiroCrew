...
| `security` | Denied commands, sensitive paths, approval posture; this-crew AgentCore Gateway catalog (live control-plane snapshot, tools/list through the workload SigV4 proxy, operator-gated target Sync — owner dashboard cookie only; **endpoint-only here: no panel calls it yet**, the Security-tab section that renders it lands with the identity settings) | `SecurityPanel.tsx`, `PostureDisclosure.tsx` | `handlers/security.py`, `handlers/tailnet.py`, `handlers/file_delivery_consent.py`, `handlers/agentcore_inspect.py` | `GET /api/security/denied-commands`, `PATCH .../builtins/{id}`, `POST .../user`, `GET,POST,DELETE /api/file-delivery/consent`, `GET /api/agentcore/gateway`, `POST /api/agentcore/gateway/verify`, `POST /api/agentcore/gateway/sync` |
