...
| `security` | Denied commands, sensitive paths, approval posture; this-crew AgentCore Gateway catalog (live control-plane snapshot, tools/list through the workload SigV4 proxy, operator-gated target Sync — owner dashboard cookie only); login-posture consent link (the pending allowlisted 3LO authorization URL, owner dashboard cookie only). **Both AgentCore surfaces are endpoint-only here: no panel calls them yet**, the Security-tab section that renders them lands with the identity settings | `SecurityPanel.tsx`, `PostureDisclosure.tsx` | `handlers/security.py`, `handlers/tailnet.py`, `handlers/file_delivery_consent.py`, `handlers/agentcore_inspect.py`, `handlers/agentcore_consent.py` | `GET /api/security/denied-commands`, `PATCH .../builtins/{id}`, `POST .../user`, `GET,POST,DELETE /api/file-delivery/consent`, `GET /api/agentcore/gateway`, `POST /api/agentcore/gateway/verify`, `POST /api/agentcore/gateway/sync`, `GET /api/agentcore/consent` |
