...
| Project selection | Pick a session's project directory: recent list, in-app directory browse with a typed path, and — on a local macOS/Windows dashboard — the host's native folder dialog | Sidebar folder config → **Select Project** → picker (Recent / Browse) | `components/ProjectPicker.tsx`, `components/FolderConfigModal.tsx` | `handlers/files.py`, `chat_handlers.py` | `GET /api/browse-dirs`, `POST /api/pick-folder`, `GET /api/project-picker/config`, `GET /api/recent-projects` |
