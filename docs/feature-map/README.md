...
| Browser panel | Live in-panel browser the agent drives; the address bar opens a real website in it on the non-native transport | Right panel → **Browser** | `components/WebPreviewPanel.tsx` | `handlers/messaging.py` | `GET,POST /api/browser/view`, `POST /api/browser/open`, `POST /api/browser/command`, `POST /api/browser/command-result` |
