...
| Chat outline | Table-of-contents of the session's user turns; click an entry to jump to and highlight that turn | Chat right panel → **+** menu → **Outline** | `pages/chat/ChatOutlinePanel.tsx` | client-only (derived from the loaded transcript; no handler) | none |
