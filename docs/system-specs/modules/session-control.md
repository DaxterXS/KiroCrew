...
A `default_action` is what the member does when the window closes unanswered,
so it requires a `deadline` — without one it would never apply and the record
would sit pending forever (`default_without_deadline`, 400).
...
then appended and surfaced like any other row and the slot is saved **forced**
(the same non-best-effort save the answer rule takes before it marks a record
answered) before the caller hears `ok`: a row that only lived in memory would
vanish on a crash and the restore-time sweep would retract the record as an
orphan while the member believed the human had been asked. A save that does not
commit closes the record as `retracted` (`retracted_reason: unsaved` — kept, not
removed, because the card already surfaced and draws its withdrawn state from
the record) and refuses (`escalation_not_persisted`, 503) so the caller can
escalate again. If the thread closes, the caller loses eligibility, or
