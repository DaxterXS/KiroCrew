...
- **Safety**: scripts must live under `~/.kiro/crew/crons/`. `is_sensitive_path()` blocks credential file access. SEL audit on every invocation. Auto-pause after 5 consecutive failures (`_AUTO_PAUSE_THRESHOLD`, single-sourced in `CronJob.record_failure`/`record_success`). The auto-pause is **persistent**: an execution-owned `auto_paused` flag (distinct from `user_paused`) is written by `_save`, propagated by `_merge_job_result`, and folded into the effective `enabled` derivation in `_load` — so a failing job stays paused across a daemon restart; `enable_job(True)` or a later success clears it (SEL-audited transitions). Concurrent execution guard prevents double-fire. The script **body** is scanned as well — at authoring time and again at every fire, via `_vet_script_file` on the freshly re-resolved path — for credential-path references, protected secret environment variables, exfiltration URLs, and the shared sensitive-path matcher. The body is Python source, not a shell command line, so it is scanned only with whole-body, source-aware detectors (credential path, secret env name, exfil URL) and is NOT routed through `is_denied` or `is_sensitive_bash_command` -- doing so produced four classes of permanent false denial on ordinary scripts (#7912, #8563, #8643, #8812); see the **Cron script bodies are not shell subjects** bullet in [security.md](security.md). The runtime control for what a script may open is the sandbox it runs in, and the scan is a first-line filter only -- it stops the obvious register-a-malicious-script case and nothing more (its own docstring says static analysis of a Turing-complete body cannot be the fence). A fire-time denial keeps the job and does not feed the auto-pause counter, so a body the scan misjudges is denied on every tick until it is edited.
- **Sandbox profile (`sandbox`, default cc)**: an UNGRANTED script cron runs under `mode="cc"` -- the same profile command jobs have always used. cc HIDES the credential stores from the child (`~/.aws` except `~/.aws/config`, `~/.kube`, `~/.gnupg`, `~/.netrc`, `~/.git-credentials`, `~/.npmrc`, `~/.pypirc`, the crew `.env`) and KEEPS `~/.aws/config` readable, so `credential_process` auth still works; `~/.ssh` stays readable as it does for a command job. The mode was `standard` before, which left `~/.aws/credentials`, `~/.kube/config` and `~/.netrc` open to an agent-written body whose only gate was the text scan above -- a contradiction, since that scan names the sandbox as the control and `open(os.path.expanduser("~/." + "aws" + "/credentials"))` walks straight past it. The wide profile survives as a per-job **operator** opt-in: `CronJob.sandbox` is a closed enum (`""` and `"cc"` both mean cc, `"standard"` widens), validated by a finite-set check at every write boundary the way `approval_mode` is, and writable ONLY from `PATCH /api/crons/{id}` (the dashboard's owner-authenticated endpoint; the Schedule page has no control for it yet) and `kirocrew cron update --sandbox {cc,standard}`. The MCP `cron_add`/`cron_update` tools deliberately do not carry the field and their schema refuses unknown keys, so a prompt-injected agent under an auto-approving session cannot widen the sandbox its own next script runs in -- the same agent-proposes/operator-disposes rule as the vault grant below. `cron_add`'s script description and success text say so, so the caller learns the child's reachable surface and that only a human can change it. A value the loader does not recognise resolves to cc rather than raising: the store is hand-editable and one bad value must not drop the record.
- **Grandfather on load**: a script record whose on-disk dict has NO `sandbox` key at all predates the field, so it keeps the `standard` profile it has always run under -- `_record_sandbox` resolves it to `"standard"` in memory, `_load` sets `_sandbox_grandfathered` and logs once at info naming only the job IDs, and the next `_save` writes the value out. ABSENCE of the key is the whole signal, which is why every record is serialized with the field even when it is `""` and why the two spellings exist. Command and agent records are untouched (a command job already ran cc; an agent job spawns no script child). An IMPORT is not an upgrade: `_sanitize_imported_crons` stamps `sandbox: ""` on any script record arriving without the key, so only a genuinely local pre-upgrade record can claim the grandfather and a foreign record cannot silently land on the wide profile. A secret-granted run ignores the field entirely and takes `strict`.
...
- **The pin binds the grant to the code the operator approved.**
  ...
  **canonical grant mapping** (re-pointing which secrets flow under an
  ...
  imports). Sibling imports are the ONLY thing an ungranted script keeps
  here: its own view is the `cc` sandbox, not the wide `standard` one it
  used to get, so the credential stores are hidden from it too and only an
  operator setting `sandbox: standard` on that job widens them (see the
  **Sandbox profile** bullet above). The granted **launcher file itself** is
  created inside that
