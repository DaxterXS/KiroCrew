...
- Ownership release on permanent session deletion: a scheduled job outlives the conversation that created it by design, so permanently deleting its owner — the single `DELETE /api/sessions/{key}` and the bulk `DELETE /api/sessions` clear, both in `dashboard/handlers/sessions.py` — RELEASES the jobs that session owns rather than deleting them. The job is work that belongs to the user and only its owner is gone: its `session_key` stamp is cleared, it keeps its schedule and stays enabled, and it moves to the CLI/Schedule-page management surface exactly as a single `kirocrew cron adopt <id> --release` leaves it. Tab close never reaches this funnel, so an archived session keeps its jobs owned. The funnel runs a LOCKED store-side owner sweep before the transcript unlink (`_owner_keys_bound_to_transcript` — a strict locked owner read against the on-disk store, never the cache-only `list_jobs()`, because a channel-created job can be owned under the key of a transcript that records no `linked_session_key`), reads the exact cron owner key of the transcript under the history lock in the same hop as the unlink (`_delete_session_capturing_owner`), and re-sweeps after the unlink commits (`_owner_keys_after_unlink`, on BOTH paths) so a channel `cron_add` that lands mid-delete is caught too; the bulk clear batches both passes for the whole selection — ONE pre-sweep over the selected rows and ONE post-unlink re-sweep over the rows it actually unlinked, two lock acquisitions rather than 2N — and merges both into the owner keys the release takes. The candidates the funnel derives on its own are the folded transcript name and, only when the deleted history key itself carries a `dashboard_` or `dashboard:` prefix, the `dashboard:` spelling of its stem (`is_dashboard_key` in `_remove_slot_for_history_key`): a channel transcript such as `slack_<ts>` gets no `dashboard:slack_<ts>` candidate, because that key belongs to a DIFFERENT, live dashboard row whose jobs the channel delete must leave owned. The post-unlink pass cannot refuse (the rows are already gone), so a store it cannot read — busy past the backoff, or unreadable — is named at WARNING with the deleted rows, the cause and the by-id remedy, and the release proceeds on what the pre-sweep found; only an add landing after that second pass is left to by-id recovery. The collected owner keys are then released in ONE atomic `CronService.release_jobs_owned_by` transaction that re-resolves ownership and principal liveness inside the store lock — a job re-adopted by another surface keeps its new owner, and the children of a `cron:<job id>` principal are released only once that job is itself gone (the same cascade `remove_job`, the batch delete, the app-owner teardown and the deferred one-shot removal apply) — matching through `cron_owner_matches` so the `cron:<job id>:<run id>` and `cron:<job id>:<agent>` spellings are covered; a save that fails after the mutation puts every touched owner back before re-raising. The cron store lock and the history lock are held sequentially, never spanned. Failure direction is split by cause. The failure modes of the store itself refuse the delete with **409** `{"ok": false, "error", "code"}` and the row stays: `cron_store_unreadable` (the store file cannot be read — the error names the path and the repair route; `cron adopt` reads the same file and is deliberately not offered), `cron_store_busy` (contention past the shared bounded backoff; retry), `cron_ownership_unknown` (single delete only: the metadata of the transcript itself is unreadable, so which jobs the delete would strand cannot be determined — the error names the candidate job ids and the by-id remedy, `kirocrew cron adopt <id> --release`, then delete again). The bulk clear refuses the whole batch by the two store causes, carrying its `cleared`/`skipped`/`failed` tallies beside the refusal; a row whose metadata is unreadable is NOT refused and NOT folded into `skipped` (which counts open tabs and pinned rows) but reported in the 200 body as `undeletable: [{id, code}]` with `cron_ownership_unknown`, so a caller can see which rows the clear left intact and why (`_delete_session_capturing_owner` raises `_OwnerKeyUnreadable` for the refusal instead of answering the `None` a pinned row answers, and the shared selector `_clearable_history_keys` returns unreadable rows as a third element). A `linked_session_key` that does not stem-match its own row is a distinct case and is not a refusal: the claim is dropped with a WARNING and the delete proceeds without releasing anything on its account (a stem mismatch is a property of the file, so refusing it made a legitimate pre-migration leftover tab permanently undeletable -- every retry and the by-id remedy read the same mismatch). Any OTHER sweep or release error does not block deletion: the delete proceeds and a WARNING names the transcript, the candidate job ids and the recovery command (`kirocrew cron adopt <id> --release`; `kirocrew cron list` shows the owners) — refusing there would let one cron-read fault block all history deletion, while the strand is bounded to the jobs named and recoverable by id. The dashboard keeps the refused row in the sidebar and renders a notice localized from `code` (`website/src/utils/historyDeleteRefusal.ts`, the `pages.chatPage.could_not_delete_cron_*` strings). Enforced by `test/test_remove_slot_for_history_key.py` (release, cascade, refusal-by-code and proceed-loudly cases), `test/test_dashboard_sessions_clear.py`, `test/test_dashboard_sessions_clearable_count.py` and `test/test_history_locking_remediation.py`; the frontend contract by `ChatSliceCoverageSecondPass.test.tsx` ("keeps a refused row in history and records the refusal by code")
