...
| `_EMBED_SPACE_KEY` | `"embedding_space"` | `store.py` | `knowledge_meta` key holding the vector space the corpus is reconciled to |
...
- **Vector leg (`_vector_search`)** — brute-force cosine over `items` with `embedding IS NOT NULL AND status='active' AND embedding_sig = ?`; returns `None` when no embedder is wired. Items whose stored embedding dimension differs from the query vector are **skipped** (not scored 0.0), with a single per-search WARNING so a stale index surfaces as "re-index needed". `_cosine_similarity` returns 0.0 for differing-length or zero vectors.
  - The `embedding_sig` predicate is the **read-side refusal** that makes an embedding-model change safe, and it is what the dimension guard cannot do: an old-space vector of the SAME WIDTH passes every length check, so without the predicate it is cosine-scored against a new-space query and returned with a confident score. A NULL signature — never stamped, or invalidated by `KnowledgeStore.reconcile_embedding_space` — reads as unproven and drops out of the leg until the sig-gated rebuild re-stamps it.
  ...
  - **The degraded mode is FTS5 + graph, and it genuinely returns results.** Zero vector candidates makes `_vector_search` return `[]`, which `_rrf_fuse` folds in as an empty leg (it skips only `None`) and `match_type` simply omits `vector` from; nothing raises and nothing short-circuits to empty. The degraded window is announced once, at invalidation time by `reconcile_embedding_space`'s WARNING (and by the watcher's `_LARGE_REBUILD_WARN_THRESHOLD` line), rather than per search — a per-search signal would cost a `COUNT` on every zero-candidate query.
...
**`KnowledgeStore.reconcile_embedding_space(signature)`** points the whole corpus at a vector space. It upserts `knowledge_meta[embedding_space]` and NULLs `embedding_sig` on every active item in **one `BEGIN IMMEDIATE` transaction** (a crash between the two steps is a lie in either order), returning the number of rows invalidated. `recorded_embedding_space()` is the non-mutating probe. Properties worth knowing:

- **O(1) per row and reversible.** It touches no `embedding` blob and re-embeds nothing; re-stamping the same signature restores the previous state exactly.
- **Idempotent** — a signature that already matches is a no-op, so a caller may run it on every boot.
- **NOT rollback-able the way `vector_memory.reconcile_embedding_space` is, structurally rather than by omission.** Memory records one space for the whole store and clears blobs, so its reconcile either happened or did not. Here the authority is per item and `rebuild_embeddings` overwrites blobs in place one at a time, so no instant exists at which the corpus is atomically in one space — an interrupted rebuild leaves some items in each, whatever reconcile did. NULLing the signature is the honest substitute: an interrupted swap is **resumable** (every not-yet-rebuilt item still reads stale and is refused meanwhile) instead of corrupt (a stale vector stamped current and skipped forever).
- **An unrecorded store is treated as unproven and invalidated**, because nothing recorded which space produced its vectors. Callers must therefore not invoke it speculatively on an upgrade path where the space is known to be unchanged. It has **no production caller yet** — it is the primitive a fleet-wide model swap needs, and the read-side refusal above is what makes it effective.
- Blocking SQLite under a write lock: call it off the event loop.
- **The fuzzy de-dup tier goes quiet for the same window, by the same rule.** `dedup._match_reason` already refuses to compare two documents whose `embedding_sig` is absent or unequal, so NULLing the signature makes the embedding tier abstain rather than compare across spaces. Tier-1 exact `content_hash` matching is unaffected, and the tier resumes as the rebuild re-stamps rows. This is one invariant enforced in two places, not two behaviours.

The store's `knowledge_meta` table (`key`/`value`/`updated_at`) holds store-level facts about the corpus as a whole; `embedding_space` is its one key today. A per-item signature can only say what produced that row, so a store-wide "which space is current" answer has nowhere else to live — and reconciliation needs it to tell a swap apart from a store that has simply never been reconciled. Added by `CREATE TABLE IF NOT EXISTS` in `_init_schema`, so an existing DB gains it on first open; it is not part of the export/import bundle.
...
- **`reconcile_embedding_space` is one transaction, idempotent, and resumable rather than rollback-able** — the space record and the signature invalidation commit together, and an interrupted swap leaves every un-rebuilt item stale-and-refused instead of stamped-and-skipped.
