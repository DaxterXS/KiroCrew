...
- DB path: `workspace_dir_for(None)/knowledge/knowledge.db` — the default workspace's data dir resolved from `config.json` (`workspaces.default.dir`), which is `config_dir()/workspace/knowledge/knowledge.db` at the shipped default; a missing DB returns "Knowledge Library is not configured…" (SEL `not_configured`).
