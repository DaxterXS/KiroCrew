...
a side turn therefore raises a permission request, and the gate judges all of
them. FAIL CLOSED: a turn whose spec cannot be derived or published is refused
...
acquisition cold-starts under the current derived spec. Closing the panel
destroys the closing generation's session outright.
