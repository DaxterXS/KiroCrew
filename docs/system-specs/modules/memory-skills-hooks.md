...
`test_member_memory_withdrawal.py` checks the complete replacement's parsed syntax
against a statically defined test function, then exercises that function through
the stopped-state sequence on a populated temporary home. It never executes
documentation text; changed instructions or extra statements fail the comparison.
It covers Global and named V1, member creation and execution refusals, preserved private files and
bindings, and admission after restoring the normal helper. OS capability discovery
and model process launch are test doubles. This regression does not establish
that a real installation's process fleet was stopped or a maintenance release
was deployed; those operator checks remain required.
...
### Owner-directed revision compaction
...
The owner can run
`kirocrew memory compact-revisions` to remove selected old accepted snapshots
from one offline V1 or V2 store. Current memory content, metadata, counters,
every proposal and the latest accepted snapshot per record remain intact.
This explicit loss of audit history does not age, downweight or evict memories.

Preview an explicitly named, existing store and UTC cutoff first:

```bash
kirocrew memory compact-revisions --store default --before 2026-08-01T00:00:00Z
```

`default` names Global V1. Use a declared store name for named V1 or a member's
V2 store. The JSON result reports `eligible` and `removed: 0`. Preview uses a
read-only SQLite connection and does not initialize or migrate the vector store.
The CLI loads configuration through the normal canonical loader. Its first load
can complete pending configuration migrations, such as adding the missing default
member; preserve the original config in the offline archive before invoking it.
Exact configuration preservation applies once those normal migrations are complete.
Unknown schema, damaged ownership and pending or failed restore state refuse.

Before applying, stop every gateway, provider, CLI writer and scheduled/task
process using the home, on every host. Disable service auto-restart. Preserve
and verify an owner-restricted offline archive of the home and external roots,
including configuration, private identities, documents and recovery state.
`--stopped` acknowledges these operator steps; it does not stop or count processes.

Use the exact eligible count from preview and a new backup filename. For a
preview reporting 12 eligible snapshots, the command is:

```bash
kirocrew memory compact-revisions --store default --before 2026-08-01T00:00:00Z --apply --expect-count 12 --stopped --backup ./memory-before-compaction.db
```

Apply makes and checks a self-contained SQLite backup, including committed WAL
data, before opening its write transaction. The backup is owner-restricted before
content is copied and cannot overwrite an existing file. This database copy is
separate from the complete offline archive. A failed command does not certify any
backup file it left behind as usable.

The command requires exclusive SQLite access and the private store's existing
lifetime lock. It checks that the source still matches the backup, the expected
count still matches, and each current revision matches its latest accepted
snapshot. Deletion runs in one transaction. The retained rows, content tables,
views, schema, identity, metadata, event history and counters must match the
backup before commit. Errors roll back the deletion. Unknown statuses, invalid
timestamps, pending/conflicting proposals and each latest accepted row survive.

Deleted pages become reusable; the command does not run `VACUUM`, shrink the file
or promise secure erasure. Optional offline `VACUUM` requires all other handles
closed, enough free space and the verified archive. Never remove WAL/SHM files
or replace a live database. Restart and refresh owner pages and edit previews
before resuming work. Check current revisions and pending proposals. Recover the
complete offline backup if verification fails before work resumes; after new
work resumes, use the supported staged restore flow.

Memory-injection diagnostics scan retained rows when requested, so purged history
no longer contributes findings. Export any needed findings before compaction.
The SEL has its own existing retention and integrity-chain rules; this command
does not edit it. The V1 accepted-history cap does not bound total storage, and
...
The F1-band report returns no interval when the sweep never reaches its requested
floor: `balanced_f1_band_98` is JSON `null`, and the text report says the floor was
not reached. Returning the best threshold twice would falsely claim that an
unattained F1 was achieved there. Existing intervals still expand only across the
contiguous qualifying points around the optimum.

The corpus checks and the metric arithmetic run in the default suite (19 tests,
...
The admission benchmark JSON/text also reports the V2 classifier on the same
measured pair cosines, explicitly excluding ranking and context-budget claims.
...
branch; it is not duplicated beside the executable benchmark. The checked-in
`eval/bench/data/member-v2-hybrid-qwen3-current.json` records the rerun after the retention change;
...
`kirocrew memory {list,search,show,stats,audit,export,migrate,import,carve,compact-revisions}` — manage memory from the command line:
- `compact-revisions --store <name> --before <UTC timestamp>` previews offline accepted-snapshot removal; explicit apply requires a stopped installation, the preview count and a new database backup. See [Owner-directed revision compaction](#owner-directed-revision-compaction).
...
- `_session_store_name(meta)` answers `""` for no key, the literal default or a
