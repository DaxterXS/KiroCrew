...
- **An SSRF vet checks the RESOLVED address, not only the name**: a name blocklist cannot see that a public name an attacker controls points at `127.0.0.1` or `169.254.169.254`, and a wildcard-DNS host needs no zone control at all. Resolution goes through one seam, refuses if ANY answer is private/loopback/link-local/reserved, refuses on failure, and runs on every redirect hop. **And the address it approved is the address that gets dialed**: the vet returns the resolved IP and the caller installs `link_unfurl.PinnedResolver` on its connector, so the client performs no second lookup for a rebinding answer to land in. `teams/client.py`, the link-preview handler, the meetings calendar provider and `wecom/media.py` all fetch that way. The pin lives in `link_unfurl.py` beside the vet whose result it serves, because every caller of the vet needs it. The residual gap is now one case and is stated rather than implied away: **a fetch through a configured operator proxy**, where aiohttp hands the proxy the hostname and resolves nothing locally, so there is no lookup for a pin to answer and the proxy's own resolution is the one that reaches a socket.
...
session on a `TCPConnector` carrying `link_unfurl.PinnedResolver`, so the socket
goes where the vet looked and a second DNS answer has nowhere to land — the same
