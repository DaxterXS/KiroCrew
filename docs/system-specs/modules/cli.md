...
| `kirocrew memory compact-revisions --store <name> --before <UTC timestamp>` | Preview old accepted revision snapshots in an existing V1 or V2 store without initializing it. `--apply --expect-count N --stopped --backup <new-file>` requires explicit offline maintenance and makes a verified SQLite backup before transactional deletion. Current memory, latest accepted revisions, proposals and counters remain intact. See [owner-directed compaction](memory-skills-hooks.md#owner-directed-revision-compaction). |
