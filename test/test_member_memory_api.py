...
@pytest.mark.asyncio
async def test_private_spawn_cannot_forge_a_global_parent(env, monkeypatch):
    from kiro_crew.dashboard.handlers import messaging

    env.state.subagents = SimpleNamespace(spawn=mock.Mock())
    monkeypatch.setattr(
        "kiro_crew.member_memory_auth.memory_request_identity",
        lambda request: ("dashboard:alice", True),
    )
    response = await messaging.api_spawn(
        request(
            env, body={"task": "read global", "parent_session": "dashboard:owner"}, internal=True
        )
    )
...
@pytest.mark.asyncio
async def test_global_internal_spawn_cannot_borrow_a_private_parent(env, monkeypatch):
    from kiro_crew.dashboard.handlers import messaging

    env.state.subagents = SimpleNamespace(spawn=mock.Mock())
    monkeypatch.setattr(
        "kiro_crew.member_memory_auth.memory_request_identity", lambda request: (None, True)
    )
    response = await messaging.api_spawn(
        request(
            env, body={"task": "borrow member", "parent_session": "dashboard:alice"}, internal=True
        )
    )
    assert response.status == 403
    env.state.subagents.spawn.assert_not_called()
...
_DOCUMENT_CREDENTIAL = "AKIAIOSFODNN7EXAMPLE"


async def _document_store(env, store: str) -> MemoryStore:
    if not store:
...
async def test_memory_events_redact_content_without_mutating_the_audit_store(env, store):
    ...
    credential = _DOCUMENT_CREDENTIAL
...
async def test_sensitive_memory_document_is_redacted_and_preserved_on_put_refusal(
    ...
    memory_store = await _document_store(env, store)
    raw = f"retain {_DOCUMENT_CREDENTIAL} exactly"
    ...
    assert _DOCUMENT_CREDENTIAL not in read_body["content"]
...
async def test_clean_memory_document_remains_editable(env, monkeypatch, store, document):
    ...
    await _document_store(env, store)
    query = {"store": store} if store else None
...
async def test_profile_put_refuses_store_removed_during_final_validation(
    ...
    memory_store = await _document_store(env, "member-alice")
...
async def test_history_put_checks_today_when_the_bounded_v2_aggregate_omits_it(env, monkeypatch):
    ...
    assert _DOCUMENT_CREDENTIAL not in baseline
...
async def test_unsigned_transcript_cannot_claim_or_mint_private_memory(env, monkeypatch):
    ...
    monkeypatch.setattr(member_memory_auth, "private_memory_execution_supported", lambda **_: True)
    with pytest.raises(memory_stores.UnknownMemoryStore, match="trusted member assignment"):
