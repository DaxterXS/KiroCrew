...
def test_every_fix_message_names_symbols_that_exist(gate: ModuleType) -> None:
    ...
    # fail. The one legitimate self-reference is this module's own name, which
    # RULE_STALE_ALLOWLIST's remedy cites, so it is added as a bare name rather
    # than by admitting the whole file.
    haystack = "\n".join(
...
def test_every_rule_has_a_probe(gate: ModuleType) -> None:
    ...
    covered.add(gate.RULE_STALE_ALLOWLIST.rule_id)
...
def test_the_allowlist_is_empty(gate: ModuleType) -> None:
    """Intended empty, so widening it is a visible edit to this line.

    The gate also prints its size on every run: an allowlist that grows quietly
    is how a diff-scoped gate stops covering the thing it was written for.
    """
    assert gate.ALLOWLIST == {}


def test_a_stale_allowlist_entry_fails(gate: ModuleType, monkeypatch: pytest.MonkeyPatch) -> None:
    """A waiver that suppresses nothing has outlived its call site."""
    monkeypatch.setattr(gate, "ALLOWLIST", {"src/kiro_crew/__init__.py": "probe"})
    assert [v.rule.rule_id for v in gate.allowlist_violations()] == ["stale-allowlist-entry"]


def test_an_allowlist_entry_for_a_missing_path_fails(
    gate: ModuleType, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A path that was deleted or renamed takes its waiver's subject with it."""
    monkeypatch.setattr(gate, "ALLOWLIST", {"src/kiro_crew/no_such_module.py": "probe"})
    assert [v.rule.rule_id for v in gate.allowlist_violations()] == ["stale-allowlist-entry"]


def test_an_allowlist_entry_cannot_waive_an_unparseable_file(
    gate: ModuleType, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A waiver about WHICH store a call reads cannot waive "nothing was checked".

    The verdict asserted is the RULE, not the exit code: an entry over a file with
    no suppressible violation is also stale, so that run exits 1 from two
    independent findings and would stay red if the unparseable one stopped firing
    entirely.
    """
    monkeypatch.setattr(gate, "REPO_ROOT", str(tmp_path))
    planted = "probe_memory.py"
    (tmp_path / planted).write_text("ctx.build_message(\n", encoding="utf-8")
    monkeypatch.setattr(gate, "ALLOWLIST", {planted: "probe"})
    survived = gate.apply_allowlist(planted, gate.scan_file(planted))
    assert [v.rule.rule_id for v in survived] == ["unparseable-source"]
    assert gate.main([planted]) == 1


def test_the_suppressibility_of_every_rule_is_an_explicit_decision(gate: ModuleType) -> None:
    """Which rules a waiver may silence is positive membership, per rule.

    "Everything except ``unparseable-source``" would hand a rule added later a
    waiver nobody granted it, and it fails toward the permissive answer — the
    shape AGENTS.md's harness-parity section forbids outright. A rule in neither
    set is a failure here and in the gate's own self-test.
    """
    ids = {r.rule_id for r in gate.ALL_RULES}
    assert ids == gate.SUPPRESSIBLE_RULE_IDS | gate.UNSUPPRESSIBLE_RULE_IDS
    assert not gate.SUPPRESSIBLE_RULE_IDS & gate.UNSUPPRESSIBLE_RULE_IDS
    assert gate.UNSUPPRESSIBLE_RULE_IDS == {"unparseable-source", "stale-allowlist-entry"}


def test_a_rule_the_tables_do_not_name_is_not_suppressible(gate: ModuleType) -> None:
    """The undecided answer is NO, which is what makes the membership positive.

    Written against a rule the tables have never heard of, because that is the
    state a newly added rule is in for as long as it takes someone to decide: a
    predicate spelled as "anything but ``unparseable-source``" answers yes here
    and hands the new rule a waiver the allowlist was never meant to give it.
    """
    invented = gate.Rule(rule_id="probe-rule-nobody-decided", message="m", fix="f")
    assert not gate.is_suppressible(invented)


def test_an_allowlist_entry_silences_its_files_call_sites(
    gate: ModuleType, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The entry does suppress, or it would be a table nobody could use."""
    monkeypatch.setattr(gate, "REPO_ROOT", str(tmp_path))
    planted = "probe_memory.py"
    (tmp_path / planted).write_text(_pool_call(), encoding="utf-8")
    assert gate.main([planted]) == 1
