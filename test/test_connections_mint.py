...
async def test_a_credential_in_the_path_leaves_the_card_unnamed(
    ...
    """The OTHER unnameable branch: for a credential-bearing PATH the raw
    sanitized_oauth_endpoint pairs a real host with the redaction tag
    ('idp.example.com', '[REDACTED: credential]'). sanitized_oauth_endpoint_display
    -- which owns the copy-ready contract -- refuses that pair (returns None)
    rather than joining it into 'idp.example.com[REDACTED: credential]', so the
    card drops rejected_endpoint and falls back to the unnamed message. This is
