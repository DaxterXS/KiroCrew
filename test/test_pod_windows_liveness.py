...
def _adoption_harness(cfg, monkeypatch, tmp_path, *, sidecar, children):
...
def test_stop_ends_the_gateways_children_after_the_gateway_itself(cfg, monkeypatch):
    ...
    monkeypatch.setattr(win, "schtasks", lambda *a: _cp())
    monkeypatch.setattr(win, "process_descendants", lambda pid: [4300, 4301] if pid == 4242 else [])
    monkeypatch.setattr(
...
def test_stop_keeps_the_pod_when_a_child_survives_the_gateway(cfg, monkeypatch):
    ...
    calls: list[str] = []
    monkeypatch.setattr(win, "schtasks", lambda *a: calls.append(a[0]) or _cp())
    monkeypatch.setattr(win, "process_descendants", lambda pid: [4300] if pid == 4242 else [])
    monkeypatch.setattr(
        win, "process_start_time", lambda pid: tokens[pid] if pid in alive else None
    )
    # `alive` is this test's model of liveness, so it must drive BOTH facets the
...
def test_stop_leaves_alone_a_stray_the_parent_map_lists_under_a_recycled_pid(cfg, monkeypatch):
    """Windows keeps a dead parent's pid on its children. When that pid is
    recycled to the gateway, an unrelated process shows up as the gateway's
    child; it was created BEFORE the gateway, which is what tells it apart, and
    it must be neither killed nor allowed to block the stop."""
    win.write_task_script(cfg, "demo")
    tokens = {4242: "1000", 4300: "1010", 7777: "900"}  # 7777 predates the gateway
    alive = {4242, 4300, 7777}
    monkeypatch.setattr(win, "schtasks", lambda *a: _cp())
    monkeypatch.setattr(win, "process_descendants", lambda pid: [4300, 7777] if pid == 4242 else [])
    monkeypatch.setattr(
        win, "process_start_time", lambda pid: tokens[pid] if pid in alive else None
    )
    # `alive` is this test's model of liveness, so it must drive BOTH facets the
