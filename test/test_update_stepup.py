...
class TestArmEndpoint:
    ...
    async def test_arm_refuses_when_the_fresh_check_finds_nothing(
        ...
        async def _retracted() -> None:
            updates._set_update_info(update_available=False, latest_version="", channel="stable")
    ...
    async def test_arm_FAILS_OPEN_when_the_fresh_check_cannot_reach_the_feed(
        ...
        try:
            ...
            # and the arm above becomes an approval command the user can no longer
            # see. Pin the restored cache, not just the returned verdict.
            assert (
