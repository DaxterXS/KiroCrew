...
class TestAdoptIsolatedKiroHome:
    ...
    def test_adoption_touches_no_filesystem(self, monkeypatch, tmp_path):
        ...
        # The memo re-prime is the ONE deliberate resolve (of the local adopted
        # path, so the first gate check does not pay it on the event loop) and has
        # its own test; here the question is adoption's own body.
        monkeypatch.setattr(paths, "_reprime_kiro_home_memos", lambda: None)
    ...
    def test_a_kiro_home_symlink_cycle_is_invalid_not_a_crash(self, monkeypatch, tmp_path):
        ...
        an abort of every CLI command: it reads as unset, and adoption proceeds."""
        _relocate_main_homes(monkeypatch, tmp_path)
        loop = tmp_path / "loop"
        ...
        assert adopt_isolated_kiro_home() == isolated_kiro_home(scratch.resolve())
...
def test_only_the_ownership_guard_consults_the_ambient_agents_dir():
    ...
    that question is asked in exactly one place -- ``agent._decline_shared_agent_home``.
    ...
    def test_a_shared_spec_still_pinning_this_home_is_named(self, monkeypatch, tmp_path, capsys):
        ...
        assert (
            "kirocrew-core" in out
            and "kirocrew-cron" not in out.split("shared spec:", 1)[1].split("\n", 1)[0]
        )
        assert "kirocrew setup --agent-only" in out
