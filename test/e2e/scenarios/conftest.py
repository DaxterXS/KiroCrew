...
branching whatsoever, which is what makes Windows a pure matrix add once
``feat/pod-windows-backend`` lands a backend for ``require_backend`` to find.
"""
...
def pod(tmp_path_factory: pytest.TempPathFactory):
    ...
    try:
        ...
        if client is not None:
            assert down.returncode == 0, f"pod down failed: {down.stderr.strip()}"
