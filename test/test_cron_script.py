...
class TestRunCommandSandboxed:
    ...
    def test_success_path_redacts_stdout(self):
        ...
        result, which is delivered to chat and persisted in cron history.
        Redaction used to run only on the non-zero-exit stderr tail, leaving
        the success path -- the one a key dump actually takes -- unredacted.
        """
        ...
        assert "BEGIN OPENSSH PRIVATE KEY" not in result["output"]
