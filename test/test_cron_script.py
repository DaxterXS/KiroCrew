...
class TestRunScriptSandboxMode:
    ...
    An ungranted script used to run ``standard``, which leaves
    ``~/.aws/credentials``, ``~/.kube/config`` and ``~/.netrc`` readable — and
    the only thing in the way was ``mcp_cron._vet_script_contents``, a static
    text scan that its own docstring says cannot be the fence
    (``open(os.path.expanduser("~/." + "aws" + "/credentials"))`` walks past
    it). The default is now ``cc``, the profile command jobs already used, and
    only an OPERATOR surface can widen one job back.
