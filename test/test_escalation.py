...
def test_escalation_is_durable_before_the_caller_hears_ok(tmp_path):
    """The card row is saved FORCED before success is returned: a row that only
    lived in memory would vanish on a crash and the restore sweep would retract
    the record as an orphan while the member believed the human was asked."""
    state = _make_state(tmp_path)
    member = _member_slot(state)
    calls: list[str] = []
...
def test_escalation_whose_save_does_not_commit_is_retracted_and_refused(tmp_path):
    state = _make_state(tmp_path)
    member = _member_slot(state)
    ...
    with pytest.raises(sc.SessionControlError) as exc:
        _escalate(state, member)
    assert exc.value.code == "escalation_not_persisted"
    assert exc.value.status == 503
    # The card row surfaced, so the record is kept and closed -- the live card
    # draws "retracted" from it -- rather than removed.
    (entry,) = conv.read_conversation(SLUG)["entries"]
    assert entry["state"] == "retracted"
    assert entry["retracted_reason"] == "unsaved"
    assert _member_needs_you(member) is False

    async def _raises():
        raise OSError("disk full")

    member._persist_for_escalation = _raises
    with pytest.raises(sc.SessionControlError) as exc:
        _escalate(state, member)
    assert exc.value.code == "escalation_not_persisted"
    assert [e["state"] for e in conv.read_conversation(SLUG)["entries"]] == [
        "retracted",
        "retracted",
    ]
    assert _member_needs_you(member) is False
...
def test_default_action_without_a_deadline_is_refused(tmp_path):
    """A default is what happens when the window closes; with no window it
    would never apply and the record would stay pending forever."""
    state = _make_state(tmp_path)
    member = _member_slot(state)
    with pytest.raises(sc.SessionControlError) as exc:
        _escalate(state, member, default_action="Push A")
    assert exc.value.code == "default_without_deadline"
    assert exc.value.status == 400
    assert member.messages == []
    assert not conv.conversation_path(SLUG).exists()
    # The pair is accepted; a default with no window is the only refusal.
    assert _escalate(state, member, deadline="30m", default_action="Push A")["ok"] is True
