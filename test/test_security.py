...
class TestSandboxDeniedCommands:
    ...
    (``config/defaults.json`` carries no ``deniedCommands`` entry); it is
...
class TestExfilUrlPathAndRawIp:
    ...
    IPv6 literal hosts must be scanned/redacted (a query-only scan or a
    letter-TLD-only host regex skips both)."""
    ...
    # ── Terminator characters in the PATH (paren-truncation bypass) ──
    ...
    def test_trailing_paren_run_is_not_stripped_wholesale(self) -> None:
        # Pins the trim against wholesale erasure: a blind
    ...
    def test_pure_trailing_paren_run_stays_in_span(self) -> None:
        # An unbalanced-closer trim written as a ``while`` loop excludes a
        # query ending in a long PURE ``)`` run — legal URL bytes that still
        # travel to the host — from the span wholesale
        # and the length heuristic sees only the short prefix. At most ONE
    ...
    def test_bare_url_trailing_paren_counts_toward_length(self) -> None:
        # Trimming the trailing ``)`` of a BARE URL (no ``(`` wrapper before
        # it) drops one legal payload byte, letting a query of exactly the
        # length threshold ending in ``)`` slip under
    ...
    def test_redaction_span_precise_for_prefix_related_urls(self) -> None:
        # Redaction must be span-precise (not global
    ...
    def test_inner_scheme_no_host_still_scanned(self) -> None:
        # The glued-URL boundary must require
        ...
        # reopened the original paren-truncation bypass. The host-bearing lookahead only
    ...
    def test_trailing_paren_trim_is_linear(self) -> None:
        # The trailing-``)`` trim must not recompute
...
class TestExfilExactHostExemption:
    ...
    def test_glued_exempt_host_span_is_not_exempt(self) -> None:
        """The boundary lookahead splits one unbroken
