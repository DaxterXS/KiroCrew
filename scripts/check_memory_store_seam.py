...
# Repo-relative path -> why this file's call sites are exempt.
#
# Intended EMPTY, and its size is printed on every run so growth is visible in
# the log rather than only in a diff. An entry that suppresses nothing is itself
# a violation (``stale-allowlist-entry``), so a converted call site cannot leave
# its waiver behind.
...
RULE_STALE_ALLOWLIST = Rule(
    rule_id="stale-allowlist-entry",
    message="allowlist entry suppresses nothing, so the waiver outlived its call site",
    fix=f"delete the entry from ALLOWLIST in {os.path.basename(__file__)}",
)
...
# Which rules an ALLOWLIST entry may silence, decided per rule and spelled as
# POSITIVE membership. "Everything except unparseable-source" would hand every
# rule added later a waiver nobody granted it, and it fails toward the permissive
# answer — a silo leak silenced by a table entry written for something else, with
# nothing going red. The self-test refuses a rule that is in neither set, so
# adding one forces the decision rather than inheriting a default.
SUPPRESSIBLE_RULE_IDS = frozenset(
    {
        RULE_STORE_REQUIRED.rule_id,
        RULE_NOT_AGENT_DERIVED.rule_id,
        RULE_NO_BARE_LITERAL.rule_id,
        RULE_NONE_STORE.rule_id,
        RULE_UNANCHORED.rule_id,
        RULE_RESOLVER_ARGUMENT.rule_id,
    }
)
...
# The other half of the suppressibility decision, listed rather than inferred. A
# waiver about WHICH store a call reads cannot waive "nothing in this file was
# checked at all", and an entry cannot waive the finding that it is itself stale.
UNSUPPRESSIBLE_RULE_IDS = frozenset({RULE_UNPARSEABLE.rule_id, RULE_STALE_ALLOWLIST.rule_id})

# Every rule, including the two that are not line shapes. The suppressibility
# decision is checked against THIS set, so a rule added to ``RULES`` without one
# fails the self-test.
...
def scan_source(path: str, source: str) -> list[Violation]:
    """Every violation in ``source``, whole-file and before any allowlist."""
...
def is_suppressible(rule: Rule) -> bool:
    """Whether an allowlist entry may silence ``rule``, by positive membership."""
    return rule.rule_id in SUPPRESSIBLE_RULE_IDS


def _suppressible(violations: Iterable[Violation]) -> list[Violation]:
    """The violations an allowlist entry is allowed to silence."""
    return [v for v in violations if is_suppressible(v.rule)]


def allowlist_violations() -> list[Violation]:
    """One violation per allowlist entry that no longer suppresses anything.

    Judged whole-file in every mode, and independently of the diff: an entry
    exists only because someone put it there, so it going stale must fail loudly
    rather than wait for a change that happens to touch its file. A path that no
    longer exists is stale for the same reason — its subject is gone.
    """
    stale: list[Violation] = []
    for path in sorted(ALLOWLIST):
        if not _suppressible(scan_file(path)):
            stale.append(Violation(path, 1, RULE_STALE_ALLOWLIST, "", (1, 1)))
    return stale


def apply_allowlist(path: str, violations: list[Violation]) -> list[Violation]:
    if path not in ALLOWLIST:
        return violations
    return [v for v in violations if not is_suppressible(v.rule)]
...
def _stale_allowlist_probe() -> tuple[bool, str]:
    """An entry that suppresses nothing must be a violation.

    Exercised here rather than as a source probe because the rule is about the
    allowlist, not about a line: it needs the module-level table swapped. Two
    shapes go stale — a file whose call sites were converted, and a file that no
    longer exists at all.
    """
    global ALLOWLIST
    saved = ALLOWLIST
    probes = (
        # No memory-context call site, so nothing for the entry to silence.
        "src/kiro_crew/__init__.py",
        # Deleted or renamed out from under the waiver.
        "src/kiro_crew/no_such_module.py",
    )
    try:
        for path in probes:
            ALLOWLIST = {path: "probe"}
            ids = {v.rule.rule_id for v in allowlist_violations()}
            if RULE_STALE_ALLOWLIST.rule_id not in ids:
                return False, f"expected a stale entry for {path}, got {sorted(ids) or 'no hit'}"
    finally:
        ALLOWLIST = saved
    return True, ""
...
def _suppressibility_probe() -> tuple[bool, str]:
    """Every rule's suppressibility is an explicit decision, not a default.

    An allowlist entry that silences a rule nobody meant it to silence fails
    toward the permissive answer, so a rule in neither set is a gate failure
    rather than an inherited waiver.
    """
    ids = {r.rule_id for r in ALL_RULES}
    undecided = sorted(ids - SUPPRESSIBLE_RULE_IDS - UNSUPPRESSIBLE_RULE_IDS)
    if undecided:
        return False, f"no suppressibility decision for {undecided}"
    both = sorted(SUPPRESSIBLE_RULE_IDS & UNSUPPRESSIBLE_RULE_IDS)
    if both:
        return False, f"{both} is listed as both suppressible and not"
    unknown = sorted((SUPPRESSIBLE_RULE_IDS | UNSUPPRESSIBLE_RULE_IDS) - ids)
    if unknown:
        return False, f"a decision names {unknown}, which is not a rule"
    return True, ""
...
def self_test() -> int:
    ...
    ok, detail = _stale_allowlist_probe()
    if ok:
        covered.add(RULE_STALE_ALLOWLIST.rule_id)
        print("  ok   stale-allowlist-entry")
    else:
        print(f"  FAIL stale-allowlist-entry: {detail}")
        failures += 1

    for label, probe in (
        ("deletion-only-hunk-is-anchored", _deletion_anchor_probe),
        ("suppressibility-is-decided-per-rule", _suppressibility_probe),
    ):
        ok, detail = probe()
        if ok:
            print(f"  ok   {label}")
...
def report(violations: Iterable[Violation], *, enforcing: bool, base: str | None) -> int:
    ...
    # Printed on every run, clean or not: an allowlist that grows silently is
    # how a diff-scoped gate stops covering the thing it was written for.
    print(f"memory-store gate: allowlist size {len(ALLOWLIST)} (intended: 0)")
...
def enforce_diff(base: str) -> int:
    ...
    """
    frm = diff_base(base)
    found: list[Violation] = list(allowlist_violations())
    for path in changed_paths(frm):
        ...
        found.extend(apply_allowlist(path, scoped))
...
def main(argv: list[str]) -> int:
    ...
    if explicit:
        found = list(allowlist_violations())
        for path in explicit:
            found.extend(apply_allowlist(path, scan_file(path)))
