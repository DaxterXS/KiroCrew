...
# path -> ((mtime, size), {name -> maps-to-local}) — reparsed only when the
# file changes.  Unlocked by design: a racing double-parse writes the same
# value, and the dict swap is atomic under the GIL.
_HOSTS_FILE_CACHE: "dict[str, tuple[tuple[float, int], dict[str, bool]]]" = {}


def _hosts_file_verdict(host: str) -> "bool | None":
    """Hosts-file verdict for *host*: True local, False remote, None absent.
