...
def _url_scan_span(match: re.Match[str]) -> tuple[str, str]:
    ...
    # proof of a wrapper, mirroring the quote rule above). A ``while`` loop
    # here erases a pure ``)`` run from the span, so the trim is one
    # conditional; the wrapper proof closes the residual byte: without the
    # ``preceding`` proof, a bare URL
