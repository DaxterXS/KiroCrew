...
def _declared_store_names() -> list[str]:
    """Every store name an audit covers, default first.

    Delegates to :func:`memory_stores.declared_store_names` rather than enumerating
    again: an audit that built its own list could disagree with the backup pass about
    which stores exist, and a store missing from one of them is a store whose contents
    that pass reports nothing about while still printing a verdict.
    """
    return memory_stores_declared_names()
...
def _memory_stores_to_scan() -> list[tuple[str, Path | None]]:
    ...
    Named stores come off :func:`_declared_store_names`, the enumeration every tier
    of the audit shares.
    ...
    for name in _declared_store_names():
...
def _lessons_files_to_scan() -> list[tuple[str, Path]]:
    ...
    :func:`_declared_store_names`. Every store is listed, including one with no
