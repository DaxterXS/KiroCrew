...
_SELF_PROTECTION_FLOOR_NOTES: dict[str, str] = {
    ...
    "sandbox-escape-ssh-self": (
        ...
        "agent sandbox. A first-seen DOTTED hostname is also refused ONCE, per process, "
        "while an off-loop DNS check rules out a loopback alias — if the host is not "
        "this machine, re-running the same command succeeds once the verdict lands. "
        "A hostname whose lookup keeps FAILING stays refused on every retry (a failed "
        "lookup is never trusted as an allow): for an ssh_config-only alias, use a "
        "name DNS can resolve, or the host's IP address, instead."
    ),
}
