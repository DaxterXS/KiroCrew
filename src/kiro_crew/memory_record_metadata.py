...
RecordKind = Literal["fact", "directive", "episode"]
...
def ensure_schema(db: sqlite3.Connection) -> None:
    ...
    statements = (
        """CREATE TABLE IF NOT EXISTS memory_record_meta (
            ...
            has_email INTEGER NOT NULL DEFAULT 0, updated_at TEXT NOT NULL)""",
        ...
        "CREATE INDEX IF NOT EXISTS idx_memory_record_category "
        "ON memory_record_meta(category, has_email, status)",
...
def get_record_metadata(db: sqlite3.Connection, record_id: str) -> dict:
    ...
    result["has_email"] = bool(result["has_email"])
    return result
