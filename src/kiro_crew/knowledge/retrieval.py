...
class HybridRetriever:
    ...
    def _vector_search(
        ...
        score. A NULL signature — an item never stamped, or one invalidated by
        ``KnowledgeStore.reconcile_embedding_space`` — is likewise unproven and
        drops out until the sig-gated rebuild re-stamps it. The KB degrades to
        FTS5 + graph rather than serving stale vectors.
