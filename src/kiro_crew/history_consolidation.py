...
def _session_store_name(meta: dict) -> str:
    """Keep V1 absence, but refuse an invalid or unavailable private identity."""
    ...
    if store in ("", DEFAULT_MEMORY_STORE):
        return ""
    return require_memory_store(store)
...
def _session_facets(meta: dict, key: str) -> "MemoryFacets":
    ...
    :func:`_session_store_name` reads (``meta["memory_store"]``): a crew alias and the
    store it binds to are separate facts. Each named member now owns a unique
    private store; recording the alias separately preserves its provenance.
