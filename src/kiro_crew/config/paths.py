...
def explicit_kiro_home() -> Path | None:
    ...
    isolated home) would be made against a home nothing actually uses.
    ...
    this runs in the CLI prologue of every subcommand, and an environment variable
    is not a place a process may be aborted from.
    """
    override = os.environ.get("KIRO_HOME")
    if not override:
        return None
...
def _reprime_kiro_home_memos() -> None:
...
def adopt_isolated_kiro_home() -> Path | None:
    ...
    VALID ``KIRO_HOME`` (an explicit choice: the operator decides which kiro home
    ...
    home. Validity is :func:`explicit_kiro_home`'s test, the same one
    :func:`kiro_home` applies: a ``KIRO_HOME`` naming ``/etc`` or a filesystem
    root is one the resolver discards, so honouring it here would leave the
    process on the shared ``~/.kiro`` after all -- it is replaced, not deferred
    to. And it is not a resolver: :func:`kiro_home` keeps reading the
    environment, so a test that isolates it with ``patch("pathlib.Path.home", ...)``
    is unaffected.
    ...
    nor resolves the adopted path. This runs before the gateway binds, and a
    probe on a roaming or UNC-backed home is a network round-trip that would
    hold readiness hostage. Whether the adopted path is a real directory is
    judged by the write guard before anything is written there.
    """
    ...
    if declared:
        logger.warning(
            "KIRO_HOME=%s names a system directory or cannot be resolved and would be "
            "ignored; adopting KIRO_HOME=%s for this non-default data home instead.",
            declared,
            adopted,
        )
    os.environ["KIRO_HOME"] = str(adopted)
    _reprime_kiro_home_memos()
    logger.info(
