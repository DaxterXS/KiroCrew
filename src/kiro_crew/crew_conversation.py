...
def mark_escalation_retracted(slug: str, escalation_id: str, reason: str) -> bool:
    """Close a pending record as ``retracted`` and say why, keeping it.

    The compensation for a card row that DID surface but whose forced save did
    not commit: the human may already be looking at the card, so the record
    stays and flips to ``retracted`` (the card draws its withdrawn state from
    that) instead of being removed, which would leave a live card with nothing
    behind it. ``reason`` lands in ``retracted_reason`` beside the orphan
    sweep's ``orphan``. Returns whether a pending record was closed.
    """
    with _lock_for(slug):
        record, state = _load_index(slug)
        if state == "unreadable":
        ...
        for entry in record["entries"]:
            if entry.get("type") != "escalation" or entry.get("id") != escalation_id:
                continue
            if entry.get("state") != "pending":
                return False
            entry["state"] = "retracted"
            entry["retracted_reason"] = reason
            _write_conversation(slug, record)
            return True
        return False
