...
Restore downloads the ``config/`` namespace and NOTHING else. Downloading every
object under ``crews/<crew>/`` instead would put every conversation the crew has
ever had on one task's disk where the backend can read all of them:
...
def run_restore(
    ...
    *,
    ...
    # the backup side uses so the two cannot drift. An enumerated set rather than a
    # `config/` namespace predicate, and the difference is a security boundary
    # rather than a tidiness preference: the task role can WRITE
