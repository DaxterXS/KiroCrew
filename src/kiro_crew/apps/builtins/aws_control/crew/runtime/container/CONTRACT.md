...
(`_flush_dirty_slots` / `_persist_open_slots` in `dashboard/chat_persistence.py`).
...
- Turn call: `POST /v1/chat/completions` with `{model, messages, id, stream}`
  (`api_completions` in `dashboard/openai_compat.py`, registered in
  `dashboard/routes/sessions.py`). Returns one JSON completion or an SSE stream.
  `id` is the slot id and is what
...
- Auth: send `X-Internal-Secret`. Both `/v1/chat/completions` and `/api/chat`
  are in `_MIXED_INTERNAL_API_PATHS` (`dashboard/server.py`), so the grant
  reaches the handler directly (`internal_path_matches` in
...
- The backend runs in dashboard mode. There is no `--no-dashboard` flag;
  dashboard-off is `--slack-only` (declared in `cli.py`), and it starts a
  different, much smaller server (`_init_api_server`) that has neither the chat
  endpoints nor the slot registry, and it quiets nothing.
...
- **`KIROCREW_TELEMETRY_DISABLED=1`** to silence the beacon, or the config key
  `telemetry.beacon_enabled=false`. Both are read by `telemetry_permitted`
  (`beacon.py`), so either gate works. An environment variable named after the
  config key is read by nothing, which is the trap: it looks like it worked.
...
- Supply no channel credentials. Every transport is gated on credentials as well
  as its flag (`_start_channel_transports` in `channels.py`), so supplying none
  keeps them all off without depending on getting a flag right.
...
- **`--approval yolo`**, and the credential. Two things the deployment cannot run
  without:
  - Approval mode is `yolo` (the `--approval` flag in `cli.py`, choices
    reads|yolo|interactive), and
  ...
  - `KIRO_API_KEY` comes from AWS Secrets Manager through the task definition.
    ...
    backend, so the key reaches the kiro-cli child (`CRED_KIRO_API_KEY` in
    `config/loader.py`) and the sandbox env filter deliberately does not deny it
    (it is absent from `_AGENT_DENIED_ENV_KEYS` in `sandbox.py`, which that
    constant's own comment states). Refuse to start when it is absent.
    **Presence is not
...
`agent.sandbox_allow_unsandboxed_exec` (declared in `config/sections.py`) is a
**config key with no
...
`start_new_session` (`acp/client.py`), so each `setsid`s into its own group and
...
1. **The transcript is not append-only.** Per-turn writes append
   (`ConversationLog.append` in `history.py`), but the whole file is atomically
   replaced,
   sometimes with a shorter one, on rotation, compaction, any metadata-line
   update, and the dashboard slot save (`history_rewrite.py`,
   `_persist_open_slots` in `dashboard/chat_persistence.py`). An offset-based
   incremental upload is
   incorrect, not merely inefficient. Upload whole objects.
2. **mtime does not change on rewrite.** `_restore_mtime` (`history_rewrite.py`)
   restores the original
   mtime after every rewrite. A sidecar polling
...
3. **Copying without the lock races a rewrite.** Writes hold a cross-process
   advisory `flock` on a per-session lock file (`_locked` in `history.py`). A copy
