...
# finishes reaping, orphaning workers that go on to finish their turn. These two
# numbers are read off the real source and confirmed against a real backend boot.
...
def _wait_for_shutdown(children: Sequence[ProcessGroup]) -> str:
    ...
    dies first. Any child dying is fatal: the backend obviously, and either other
    child too, because the task cannot do its job without them.
    """
...
def verify_layout(settings: Settings) -> None:
    ...
    directly under that root (``chat_persistence.py``, ``run_marker.py``, confirmed
    by booting a real gateway). The backend is launched with
