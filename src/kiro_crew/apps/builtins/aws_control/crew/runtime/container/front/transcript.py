...
Boot downloads the two authority files and no conversations at all (see
``EPHEMERAL-CONTRACT.md``), so a task starts with no transcripts on disk. This
module is the other half of that split: before a turn reaches the backend, the one
transcript that turn continues is fetched.
