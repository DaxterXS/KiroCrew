...
def _open_nofollow_under(root: Path, path: Path) -> int:
    ...
    Opening ``root`` normally is not good enough, and the tempting justification --
    "it is the container's own data home, fixed by the task definition, not something
    the agent names" -- is true of the PATH and false of what the path resolves to: the roots
