    ...
    async def turn(request: Request):
        ...
        async with fake.lock:
            if sid in fake.inflight:
                fake.saw_409 += 1
                # Real backend shape, verified against a live backend: OpenAI-style error.
                return JSONResponse(
...
# --------------------------------------------------------------------------- #
# Stream projection — OpenAI-format frames (VERIFIED against a real backend:
# /v1/chat/completions streams `data: {chat.completion.chunk}` +
