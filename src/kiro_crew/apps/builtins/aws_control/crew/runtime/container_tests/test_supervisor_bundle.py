...
def test_a_bundle_upgrade_prunes_a_removed_skill(tmp_path):
    # Replace, not merge: a skill present from a prior install but absent from the new
    # bundle must be GONE after install, or a removed (possibly governance-relevant) skill
    # stays active on the persistent volume. Reddens against a copy-into-place merge.
    build_bundle(tmp_path, crew_name="frontdesk")
    settings = make_settings(tmp_path, crew_name="frontdesk")
    # Simulate a prior install leaving a skill the new bundle does not carry.
    stale = settings.data_home / "skills" / "obsolete"
    stale.mkdir(parents=True, exist_ok=True)
    (stale / "SKILL.md").write_text("# Obsolete\n", encoding="utf-8")

    bundle_mod.install_bundle(settings, agents_dir=tmp_path / "kiro" / "agents")

    # The new bundle's skill landed, and the stale one was pruned by the atomic replace.
    assert (settings.data_home / "skills" / "greet" / "SKILL.md").is_file()
    assert not (settings.data_home / "skills" / "obsolete").exists()


def test_no_skills_staging_dir_is_left_behind(tmp_path):
    build_bundle(tmp_path, crew_name="frontdesk")
    settings = make_settings(tmp_path, crew_name="frontdesk")
    bundle_mod.install_bundle(settings, agents_dir=tmp_path / "kiro" / "agents")
    leftovers = list(settings.data_home.glob(".skills-staging-*"))
...
def test_a_nested_symlink_in_the_old_skills_tree_is_removed_not_followed(tmp_path):
    # Under stage-then-atomic-replace, a symlink an agent planted inside the PRIOR skills
    # tree is discarded with that tree (rmtree does not follow it) rather than being written
    # through. The install lands the fresh tree and never touches the outside victim.
    build_bundle(tmp_path, crew_name="frontdesk")
    settings = make_settings(tmp_path, crew_name="frontdesk")
    victim = _outside_target(tmp_path)
