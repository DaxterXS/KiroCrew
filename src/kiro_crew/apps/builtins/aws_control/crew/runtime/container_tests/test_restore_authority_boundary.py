"""Two authority-boundary properties, each pinned so reverting it reddens.

Both sit next to an existing guard, which is why they are worth naming precisely
rather than filing as "hardening".

* Restore must choose which objects to write by an ENUMERATED set, not a namespace
  predicate. The task role can write to this bucket in persistent mode, so under a
  namespace test any object it uploads below ``config/`` is restored on the next
  boot -- including one that governs the agent itself. The sibling test
  ``test_only_the_config_namespace_is_written`` says "namespace" in its own name and
  passes either way, because a namespace is what that assertion describes.

* The transcript write must not run on the event loop, one line below a comment
