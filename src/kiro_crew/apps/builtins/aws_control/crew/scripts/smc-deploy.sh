...
smc_main() {
  ...
  MEMORY="chatbot"
  # Whether the value above came from a FLAG or from that default. A resume needs
  # to tell those apart: every step passes Memory to `cloudformation deploy` as an
  # explicit parameter override, so a `--from N` re-run that defaulted would hand a
  # live persistent stack `Memory=chatbot` and take its S3 grant away. See
  # `adopt_recorded_deploy_shape`.
  MEMORY_GIVEN=0
  ...
  TRUST_DOMAIN=""
  # Same resume problem as MEMORY_GIVEN, for the same reason: both of these ride to
  # `cloudformation deploy` as explicit parameter overrides, so a resume that
  # defaulted would relax a live stack's isolation statement or flip its sandbox
  # posture without the operator naming either.
  SANDBOX_GIVEN=0
  TRUST_DOMAIN_GIVEN=0
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
     ...
     rather than after five steps of work." ;;
      ...
      --trust-domain) TRUST_DOMAIN="$2"; TRUST_DOMAIN_GIVEN=1; shift 2
                 case "$TRUST_DOMAIN" in single-principal) : ;;
                   *) die "--trust-domain takes single-principal, got '${TRUST_DOMAIN}'." ;;
                 esac ;;
      --memory-mode)  MEMORY="$2"; MEMORY_GIVEN=1; shift 2
     ...
     rejects anything else too, but it would say so only after five steps of work." ;;
      ...
      --require-sandbox) ALLOW_UNSANDBOXED=false; SANDBOX_GIVEN=1; shift ;;
  ...
  param() { # STACK KEY -> the stack's CURRENT parameter value ("" if absent)
    # The live stack is the authority on how this crew was deployed. A resume whose
    # work dir is gone -- and it lives under scratch, so that is ordinary rather
    # than exotic -- reads the shape back from here instead of applying a default.
    aws_ cloudformation describe-stacks --stack-name "$1" \
      --query "Stacks[0].Parameters[?ParameterKey=='$2'].ParameterValue" --output text 2>/dev/null
  }
  stack_status() {
    aws_ cloudformation describe-stacks --stack-name "$1" \
  ...
  dry_aws() {
    ...
    case "$svc $op" in
      ...
      "cloudformation describe-stacks")
        ...
        # A PARAMETER query is a different question from an output query: it asks how
        # the live stack was deployed. SMC_FIX_STACK_PARAM_<Key> answers one, and an
        # unset variable means the stack does not carry that parameter -- which is
        # what a first deploy looks like, and what makes the default safe there.
        local pkey; pkey="$(printf '%s\n' "$@" | sed -n "s/.*ParameterKey=='\\([^']*\\)'.*/\\1/p")"
        if [ -n "$pkey" ]; then
          local _fixvar="SMC_FIX_STACK_PARAM_$pkey"
          printf '%s' "${!_fixvar:-}"
          return 0
        fi
  ...
  # --- the deploy SHAPE must survive a resume ------------------------------
  #
  # Memory, TrustDomain and AllowUnsandboxedExec each reach `cloudformation deploy`
  # as an explicit `--parameter-overrides` value, and each has a default. So a
  # resume does not merely forget them, it OVERWRITES them. The documented `--from
  # N` re-run (the usage block and every resume hint this script prints) does not
  # repeat the mode flags, so a persistent crew resumed at step 6 is handed
  # `Memory=chatbot`, the task role loses its S3 action, and new conversations stop
  # being backed up. Nothing fails at the time; the next task replacement is where
  # the conversations go.
  #
  # Four rules, in order of authority:
  #
  #   * flag given          -> the flag wins. The operator is stating intent.
  #   * recorded in $WORK   -> adopt the record.
  #   * live stack has it   -> adopt the stack. $WORK is under scratch and gets
  #                            reclaimed, so a recordless resume is ORDINARY, and
  #                            the deployed stack is the real authority anyway.
  #   * no stack either     -> the default is safe: there is nothing to overwrite.
  #
  # Reading the stack is why this sits here rather than beside the argument parser:
  # `param()` needs the AWS wrappers, and the account this would read from is only
  # confirmed a few lines above.
  _shape_file() { printf '%s/shape-%s' "$WORK" "$1"; }
  _adopt_shape() {   # flag-name, given?, current, stack-parameter-name -> value to use
    local name="$1" given="$2" current="$3" key="$4" file live
    file="$(_shape_file "$name")"
    if [ "$given" -eq 1 ]; then
      printf '%s' "$current" > "$file"; printf '%s' "$current"; return 0
    fi
    if [ -s "$file" ]; then cat "$file"; return 0; fi
    live="$(param "$CREW_STACK" "$key")" || live=""
    case "$live" in ""|None) live="" ;; esac
    if [ -n "$live" ]; then
      printf '%s' "$live" > "$file"; printf '%s' "$live"; return 0
    fi
    printf '%s' "$current" > "$file"; printf '%s' "$current"
  }
  MEMORY="$(_adopt_shape memory-mode "$MEMORY_GIVEN" "$MEMORY" Memory)"
  TRUST_DOMAIN="$(_adopt_shape trust-domain "$TRUST_DOMAIN_GIVEN" "$TRUST_DOMAIN" TrustDomain)"
  ALLOW_UNSANDBOXED="$(_adopt_shape allow-unsandboxed-exec "$SANDBOX_GIVEN" \
    "$ALLOW_UNSANDBOXED" AllowUnsandboxedExec)"
  if [ "$FROM" -gt 0 ] && [ "$MEMORY_GIVEN" -eq 0 ]; then
    ok "memory mode $MEMORY carried into this resume"
  fi
  ...
  # refusing here means the owner learns it before any stack work rather than
  # watching six steps reach CreateChangeSet and fail.
  #
  # It runs AFTER the shape is adopted rather than at argument-parse time: a resume
  # that recovers `persistent` has to face this check too, and a version placed
  # before the adoption would wave through exactly the resumes that matter.
  #
