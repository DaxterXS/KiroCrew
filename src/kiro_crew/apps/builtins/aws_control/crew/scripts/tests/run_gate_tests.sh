...
# ---------------------------------------------------------------------------
# The deploy SHAPE survives a resume.
#
# Memory rides to `cloudformation deploy` as an explicit parameter override and it
# has a default, so a resume that does not repeat --memory-mode does not merely
# forget the mode, it OVERWRITES a live persistent stack with chatbot: the task
# role loses its S3 action and new conversations stop being backed up, silently.
# Every resume hint this script prints tells the operator NOT to repeat the flag,
# so this is the documented path rather than a corner.
#
# Driven end to end, and asserted three ways because the fix has three sources of
# truth in priority order.
_SW="$(mktemp -d)"
KIRO_API_KEY='aws-kiro-abcdefghijklmnopqrstuvwxyz0123456789' \
  SMC_WORK="$_SW" \
  bash "$SCRIPT" --dry-run --crew acme-support \
    --memory-mode persistent --trust-domain single-principal >/dev/null 2>&1
assert_eq "shape: step 0 records the resolved memory mode" \
  "persistent" "$(cat "$_SW/shape-memory-mode" 2>/dev/null)"
assert_eq "shape: step 0 records the resolved trust domain" \
  "single-principal" "$(cat "$_SW/shape-trust-domain" 2>/dev/null)"

# THE REGRESSION: resume with no mode flags. Reverting the fix makes this chatbot.
out="$(KIRO_API_KEY='aws-kiro-abcdefghijklmnopqrstuvwxyz0123456789' \
  SMC_WORK="$_SW" \
  bash "$SCRIPT" --dry-run --crew acme-support --from 6 2>&1)"
assert_contains "shape: a resume carries the recorded memory mode forward" \
  "$out" "memory mode persistent carried into this resume"
assert_eq "shape: a resume leaves the record on persistent, not the default" \
  "persistent" "$(cat "$_SW/shape-memory-mode" 2>/dev/null)"
assert_not_contains "shape: a resume never silently applies the chatbot default" \
  "$out" "memory mode chatbot carried"

# An explicit flag still wins over the record: the operator is stating intent.
out="$(KIRO_API_KEY='aws-kiro-abcdefghijklmnopqrstuvwxyz0123456789' \
  SMC_WORK="$_SW" \
  bash "$SCRIPT" --dry-run --crew acme-support --memory-mode chatbot --from 6 2>&1)"
assert_not_contains "shape: an explicit flag is not reported as carried" \
  "$out" "carried into this resume"
assert_eq "shape: an explicit flag is re-recorded for the next resume" \
  "chatbot" "$(cat "$_SW/shape-memory-mode" 2>/dev/null)"
rm -rf "$_SW"

# The live-stack fallback in the driver (`param()` reads the deployed stack when no
# record survives) is deliberately NOT asserted here: wiring its dry-run fixture
# needs edits to this file's credential-bearing lines. It is code without a gate,
# and saying so is better than implying it is covered.
