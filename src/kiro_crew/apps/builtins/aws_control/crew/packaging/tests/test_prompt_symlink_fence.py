...
The hole this closes, stated as the two calls that disagree:
...
so a symlink inside the agents directory pointing at ``~/.kube/config`` slips past
a fence written specifically to refuse that file, and the read follows the link.
Checking the resolved target is what makes the two agree.
