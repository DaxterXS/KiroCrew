...
def test_MUTATION_sensitive_path_fence(tmp_path):
    ...
    bad = load_build(
        # Anchored to the CURRENT guard line, which checks the resolved target as
        # well as the path as written: a symlink walks past the link-only form. See
        # test_prompt_symlink_fence.py.
        mutate=(
