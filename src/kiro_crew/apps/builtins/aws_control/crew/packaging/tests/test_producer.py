...
def _child_env() -> dict:
    ...
    import path. It gets that from ``PYTHONPATH`` rather than from
    ``cwd=CREW_ROOT``: both resolve the module identically, but ``PYTHONPATH`` lets
    the child run from a temp cwd, so any bytecode it writes lands under that temp
    dir and is reclaimed with it instead of dropping ``__pycache__`` directories
    into the source tree.

    CREW_ROOT is PREPENDED so this directory's ``packaging`` shadows the unrelated
    PyPA ``packaging`` distribution for the child, which is the precedence a cwd
    inside the crew root would give. ``PYTHONDONTWRITEBYTECODE`` is a
    belt-and-braces second guard: even the temp-cwd imports write no ``.pyc`` at all.
    """
    env = dict(os.environ)
...
def load_build(mutate: tuple[str, str] | None = None) -> types.ModuleType:
    ...
    exec, disabling exactly one guard so the leak it prevents becomes observable.
    """
...
def test_MUTATION_signature(tmp_path):
    ...
    # With merge's refusal disabled, verify still catches it -- unless verify's own
    # signature line is disabled too, which is the real guard under test here.
...
def test_MUTATION_credential_scan(tmp_path):
    """Disable scan_text; nothing blocks the credential skill and it ships."""
    src = make_crew(
        tmp_path / "home",
        skills={"leaky": {"SKILL.md": f"# Leaky\nkey = {FAKE_AWS_KEY}\n"}},
    )
...
def test_MUTATION_credential_filename(tmp_path):
    """Disable refused_by_name; nothing blocks a skill carrying a .env."""
    src = make_crew(
        tmp_path / "home",
        skills={"withenv": {"SKILL.md": "# ok", ".env": "SECRET=hunter2"}},
    )
...
# the source tree. Running it with cwd=CREW_ROOT is what drops __pycache__ dirs
# under the crew tree, and that residue outlives the test.
# ---------------------------------------------------------------------------
