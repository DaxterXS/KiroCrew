...
- `BundleObjectKey`, the former BundleDigest crew-stack parameter and the `CREW_BUNDLE`
...
- Templates: remove `BundleObjectKey`, `CREW_BUNDLE`, the bundle-read task-role
  statement, and any remaining BundleDigest crew parameter. `check_param_seam.sh`
  must stay
