...
def _warn_home_unreadable_beneath_authority_once(home_path: Path, error: Exception) -> None:
    """Say once per process that an unusable home file was skipped beneath the authority.

    Unusable covers every shape ``_subordinate_ceiling`` skips: a read error, a JSON
    error, and a document ``parse_policy`` rejects. The authority still governs, so
    nothing loosened; but a local operator who meant that file to tighten the
    ceiling must be able to see that it did not.
    """
    if _process_state.home_unreadable_warned:
        return
    _process_state.home_unreadable_warned = True
    logger.warning(
        "security policy at %s is unusable (%s); skipped beneath the central "
...
def load_security_policy(
    ...
    **advisory by default**, so existing installs and policy files keep working
    ...
    # that env or central would have governed with the home file ignored.
    if declared is None:
        try:
