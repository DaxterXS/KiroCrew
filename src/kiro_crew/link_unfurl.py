...
1. :func:`vet_unfurl_url` — the SSRF vet. It is the *only* place that decides a
   ...
   nothing.
2. :class:`PinnedResolver` — the other half of that close. It is an aiohttp
   resolver that answers with the address the vet already approved, so the
   socket cannot be opened to anything else. It lives here, beside the vet whose
   result it serves, because every caller that fetches a vetted URL needs it:
   the link-preview handler, WeCom media, and any channel added later.
...
class PinnedResolver(aiohttp.abc.AbstractResolver):
    ...
    Construct it from a :class:`VettedUrl`'s ``wire_host``, ``ip`` and ``port``,
    and install it on the connector of a session used for that ONE URL::
