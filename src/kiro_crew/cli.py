...
""",
    ...
    mem_compact = mem_sub.add_parser(
        "compact-revisions", help="Preview or compact old accepted revisions in an offline store"
    )
    mem_compact.add_argument(
        "--store", required=True, help="Exact store name; use default for Global"
    )
    mem_compact.add_argument(
        "--before", required=True, help="Exclusive ISO timestamp cutoff in UTC"
    )
    mem_compact.add_argument(
        "--apply", action="store_true", help="Delete the previewed old snapshots"
    )
    mem_compact.add_argument(
        "--expect-count", type=int, help="Required exact preview count for apply"
    )
    mem_compact.add_argument(
        "--stopped",
        action="store_true",
        help="Confirm all users and service restarts are stopped and an offline archive is verified",
    )
    mem_compact.add_argument(
        "--backup", type=Path, help="New SQLite backup file required for apply"
    )
