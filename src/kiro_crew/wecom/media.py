...
def _vet_media_url(url: str) -> link_unfurl.VettedUrl:
    ...
    session on a :class:`link_unfurl.PinnedResolver` built from ``wire_host``,
    ``ip`` and ``port``. **Under a configured operator ``proxy`` the pin stays
    ...
    try:
        if proxy:
            ...
            connector = aiohttp.TCPConnector(
                resolver=link_unfurl.PinnedResolver(vetted.wire_host, vetted.ip, vetted.port),
                limit=1,
                # The pinned resolver already returns a literal with its family,
                # so aiohttp must not narrow or re-derive it: constraining the
                # family here would either drop a valid IPv6 target or re-open the
                # door to a second lookup.
