...
class KasLoginService:
    ...
    async def _persist_and_finish(self, login_id: str, token: KasToken) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            if result.get("status") == "authorized" and replaces and replaces != token.identity:
                # An account switch across slots. The store resolves by fixed slot
                # priority (external_idp > builder_id > identity_center > social),
                # not by recency, so a Google sign-in landing under a still-stored
                # Identity Center entry would leave the agents on the OLD account
                # while the card claimed the switch happened. Remove the slot the
                # user asked to replace -- only now, after the new credential is on
                # disk, so a failed sign-in never leaves the vault empty. A removal
                # that fails is reported, not hidden: the re-read status will still
                # show the previous account, and Sign out clears it by hand.
                try:
                    await asyncio.to_thread(self._store.delete, replaces)
                    result["replaced"] = replaces
                except TokenStoreError as err:
                    logger.warning("could not remove replaced KAS identity %s: %s", replaces, err)
