...
def run_command_sandboxed(
    ...
    try:
        ...
        # job result -- delivered to chat and persisted in cron history. Only the
        # non-zero-exit stderr tail below used to be redacted, so the success
        # path was the unredacted one. Redact the WHOLE text BEFORE the 64KB
        # slice, for the same reason as that stderr tail: slicing first could cut
        # off a credential's detectable prefix (e.g. the BEGIN line of a PEM key,
        # or the scheme of a token-bearing URL), letting the raw secret tail
        # through redaction.
