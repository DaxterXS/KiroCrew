...
class AutoNudgeService:
    ...
    def __init__(
        ...
        self._deferred_monitor_replacements: dict[str, NudgeLoop | None] = {}
            ...
            async with self._lock:
                ...
                if existing is not None:
                    ...
                    if defer_replaced_trust_revocation:
                        self._deferred_monitor_replacements[loop.id] = deferred_prior
                    else:
                    ...
                    self._deferred_monitor_replacements[loop.id] = None
                self._loops[loop.id] = loop
    ...
    def commit_monitor_replacement(self, loop_id: str) -> None:
        ...
        prior = self._deferred_monitor_replacements.pop(loop_id)
        ...
        async with _maintenance_lock(self._base_dir):
            async with self._lock:
                ...
                try:
                    ...
                    self._deferred_monitor_replacements[loop_id] = prior
                    raise
    ...
    def remove_sync(
        ...
        deferred_prior = self._deferred_monitor_replacements.pop(loop_id, None)
        if deferred_prior is not None:
