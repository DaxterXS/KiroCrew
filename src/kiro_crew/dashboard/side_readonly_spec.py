...
def publish_readonly_spec(base_name: str, project_dir: str | None = None) -> PublishedSpec:
    ...
    Returns the derived agent NAME the side session must be created with, the
    content digest, and whether a write happened. Reads the base spec (project
    ...
    return PublishedSpec(name=derived_name, digest=digest, wrote=wrote)
