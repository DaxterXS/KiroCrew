...
async def internal_memory_scope(
    ...
    try:
        ...
        if actual is not None:
            if actual != request.headers.get("X-Session-Key", ""):
                raise ValueError("session header mismatch")
            if claimed_session is not None and claimed_session != actual:
                raise ValueError("claimed session mismatch")
            store = await asyncio.to_thread(memory_request_bound_store, request)
            if store is None:
            ...
            return store if private else None, None
...
async def require_private_memory_session(
    ...
    actual, verified = await asyncio.to_thread(memory_request_identity, request)
    if verified and actual is None and not private:
        return None
    if (
        not verified
        and not private
    ...
    if (
        ...
        and actual
        and actual == declared
        and (session_key is None or session_key == actual)
    ):
        try:
            bound = await asyncio.to_thread(memory_request_bound_store, request)
        except (ValueError, OSError):
            bound = None
        if bound == store:
            return None
