...
async def escalate_to_user(
    ...
    * **Escalation is not approval.** This path carries a decision the member
    ...
    if default_clean and not deadline_iso:
        # A default is "what happens when the window closes unanswered"; with
        # no window it never applies, and the record would sit pending forever
        # while telling the human a default is coming. Refuse rather than
        # store a promise nothing can keep.
        raise SessionControlError(
            "default_action requires a deadline: the default applies when the "
            "window closes, so name the window",
            code="default_without_deadline",
            status=400,
        )
    ...
    # row is appended and surfaced like any other, then the slot is saved
    # FORCED before the caller hears "ok": an accepted escalation whose row
    # only ever lived in memory would vanish on a crash, and the roster's
    # restore-time sweep (``crew_conversation.reconcile_with_transcript``)
    # would then retract the pending record as an orphan — the card, the bell
    # and the badge all gone while the member believes the human was asked.
    # A save that does not commit retracts the record and refuses, so the
    # caller can escalate again instead of waiting on a decision nobody sees.
    try:
        append_and_surface(
    ...
    # The same durability hop the answer rule takes before it marks a record
    # answered (``_ChatSlot._persist_for_escalation`` -> forced, non-best-effort
    # save); a slot built without the hook falls back to the state method.
    persist = getattr(home, "_persist_for_escalation", None)
    try:
        if callable(persist):
            persisted = bool(await persist())
        else:
            persisted = bool(await state._persist_slot_for_escalation(home))
    except Exception:  # noqa: BLE001 - any save failure is the same refusal
        logger.warning("escalate_to_user: transcript save failed for %s", home.key, exc_info=True)
        persisted = False
    if not persisted:
        # The row surfaced (the human may be looking at it), so the record is
        # KEPT and closed as retracted -- the card draws its withdrawn state
        # from that -- rather than removed, which would leave a live card
        # with nothing behind it and nothing to clear ``needs_you``.
        def _retract_unsaved() -> None:
            if not slug:
                return
            try:
                conv.mark_escalation_retracted(slug, escalation_id, "unsaved")
            except Exception:  # noqa: BLE001 - compensation is best-effort; logged
                logger.warning("escalation retract (unsaved) failed for %s", slug, exc_info=True)

        await asyncio.to_thread(_retract_unsaved)
        raise _escalation_denied(
            caller_session_key,
            "the escalation could not be written to the thread's transcript; "
            "it was withdrawn and nobody was asked — escalate again",
            code="escalation_not_persisted",
            status=503,
        )
