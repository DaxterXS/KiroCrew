...
def _redact_field(value: object, *, clamp: bool = True) -> str:
    ...
    # pulls in the chat-slot machinery, which a crew read does not need. Same
    # deferral as _cap_str, and the same redaction owner: redact_peer_text is the
    # exfiltration-URL + credential chain in the repo's fixed order.
    from kiro_crew.dashboard.remote_relay import redact_peer_text

    value = redact_peer_text(value)
    if clamp:
        value = value[:_PEER_FIELD_MAX_CHARS]
    return value
