...
def _apply_default_approval_mode(state: "DashboardState", slot: "_ChatSlot", mode: str) -> bool:
    """Put a BRAND-NEW slot on the tier stored in ``default_approval_mode.json``.
    ...
    * ``trust_reads`` -> ``_trust_reads`` only, session policy left ``""``
    ...
    What is NOT replicated, and why. Both used to be justified by a claim about
    BIRTH STATE -- "a new slot has no binding yet" -- and that claim was false:
    ``get_or_create_slot`` resolves a channel-shaped requested key and can set
    ``linked_session_key`` at creation (state.py, the ``is_channel_session_key``
    branch). A statement about WHEN a slot was made is a proxy; the field is the
    property, so the caller now tests the field and this helper is only ever
    reached for an unbound slot:

    * the sharing-propagation loop, which exists so that several slots resolving
      to ONE effective session key cannot disagree. The caller requires an empty
      ``linked_session_key``, so this slot shares its effective key with nothing
      and there is no peer to disagree with.
    * Slack channel-trust propagation, which keys off ``slot._slack_channel``.
      The same caller-side condition excludes a slot bound to a channel session,
      which is the only way a dashboard-created slot acquires one at birth.
...
async def api_chat_slot_create(request: web.Request) -> web.Response:
    ...
    if not request_app and not instance_id and _owner_request and _mode in _TIER_INHERITING_MODES:
        # Read from the KEYSTONE leaf, never from `config.json`. The tier is a
        # security ceiling, and `config.json` is write-caught on the agent tool path
        # but NOT on the shell path -- `security.py` carries the verification that
        # `echo x > .../config.json` is not caught -- so a tier stored there would be
        # self-grantable by any auto-approved agent shell. `default_approval_mode.json`
        # is on `_CREW_SECRET_LEAVES`, which covers both paths. There is deliberately
        # NO `config.json` fallback on this path: a fallback would keep the feature
        # working while ungated, which is worse than the feature being inert.
        try:
            _default_approval_mode = await asyncio.to_thread(default_approval_state.load_mode)
        except Exception:
            logger.warning(
                "Reading the default-approval-mode keystone failed; new session "
                "starts on the interactive floor",
                exc_info=True,
            )
            _default_approval_mode = "normal"
    ...
    with state.suspend_slots_push():
        ...
        # ONLY the two refusals this branch can actually reach. `_default_approval_mode`
        # is non-"normal" only when the read gate above already passed, so an
        # app-token caller, a remote-bound create, a non-owner and an app-worker mode
        # cannot appear here: they never resolved a tier in the first place, so there
        # is nothing for them to be refused. `tier_not_persistable` cannot appear
        # either -- the loader clamps on read, so a stored `trust`/`yolo`/junk value
        # arrives as "normal". Naming those five anyway would assert coverage this
        # function does not have, which is worse than naming none: a reader would
        # conclude the non-owner path is observable when it is silent by construction.
        # That gap is stated in the PR body and in config.md rather than papered over.
        _tier_refusals = []
        if _default_approval_mode != "normal":
            # The read runs before newness is known (the hoist that closed the TOCTOU),
            # so a RESUMED slot resolves a tier and is then correctly refused one.
            if not is_new_slot:
                _tier_refusals.append("not_a_new_session")
            # A concurrent app-token create won this key, so `get_or_create_slot`
            # handed back ITS slot. Rare, and the only principal refusal reachable
            # here -- the owner passed every gate, then lost the race.
            if slot._app:
                _tier_refusals.append("slot_owned_by_an_app")
        if _tier_refusals:
            logger.info(
                "default_approval_mode=%s (keystone) not applied to %s: %s",
        ...
        if (
            ...
            and not slot._app
            # The FIELD, not a claim about birth state. `get_or_create_slot` resolves a
            # channel-shaped requested key and binds `linked_session_key` at creation,
            # so "a new slot is unbound" is false. A bound slot's conversation IS a
            # channel session: applying a tier there would put it on a slot whose
            # effective key is shared, without the sharing-propagation loop that keeps
            # co-resolving slots from disagreeing -- and channel trust keys off
            # `_slack_channel`, which this path does not touch.
