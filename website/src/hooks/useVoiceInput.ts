...
interface Opts {
   ...
   * owner appears instead of letting it drop. Defaults to `!ownsSession`: an
   * instance with no ownership predicate is the legacy single consumer and
...
export function useVoiceInput(onText: (text: string, sessionId: string | null, origin: TranscriptOrigin) => void, opts: Opts = {}) {
  ...
  // An instance that declares no ownership at all is the legacy single
  // consumer: it must keep receiving every transcript, so it accepts unowned
  // text unless told otherwise. Only an instance that CAN say whose composer it
  // is (`ownsSession`) is held to the claim protocol.
  const acceptsUnownedRef = useRef(opts.acceptsUnowned ?? !opts.ownsSession)
  acceptsUnownedRef.current = opts.acceptsUnowned ?? !opts.ownsSession
