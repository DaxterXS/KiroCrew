...
export default function SelectionToolbar({ containerRef, actions, externalSelection, composer, suspended = false }: SelectionToolbarProps) {
  ...
  useLayoutEffect(() => {
    ...
    // Flip above the anchor when it would overflow the bottom edge. The
    // composer's anchor is the selection's BOTTOM, so its flipped position is
    // measured from the selection's TOP (otherwise the box lands on the text).
    const flipBase = composer && selectionRectRef.current ? selectionRectRef.current.top - 8 : pos.y
    const top = pos.y + h + margin > vh ? Math.max(margin, flipBase - h - margin) : pos.y
  ...
  }, [visible, pos, composerGrowTick, composer, copyFailed])
  ...
  return (
    ...
    >
      <textarea
        ...
        className={`bg-bg border border-border rounded-md px-2.5 py-1.5 text-text text-[13px] font-body outline-none focus-ring resize-none leading-[20px] overflow-hidden ${stacked ? 'w-full' : 'flex-1 min-w-[140px]'}`}
      />
      {controls}
