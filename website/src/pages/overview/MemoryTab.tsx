...
export default function MemoryTab({ refreshTrigger }: { refreshTrigger: number }) {
  ...
  const deleteLessonRow = async (rule: string, repoScope: string | null) => {
    ...
    // The delete is durably persisted at this point; loadLessons() is a display
    // refresh. Await it and route a failed refresh through the error surface so
    // a rejected GET is not silently swallowed (the row would look un-deleted).
    try {
      await loadLessons()
    } catch (e) {
      setLessonError(e instanceof Error ? e.message : String(e))
