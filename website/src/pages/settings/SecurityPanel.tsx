...
import { api, ApiError, type DefaultApprovalModeData, type DeniedCommandsData, type DeniedCommandRule, type DeniedUserRule, type GovernanceDistributionData, type GovernancePolicyData, type GovernanceScope, type GovernanceScopeDetail, type SecurityPostureData, type TailnetStatusData, type TrustedAppsData } from '../../api/client'
...
/** Own query key for the dedicated owner-only endpoint, distinct from the
 *  generic `kirocrewConfig` key YoloDurationCard uses. */
const DEFAULT_APPROVAL_MODE_QK = ['default-approval-mode'] as const
 ...
 *  with the `data-setting-label` deep-link anchor, and the same pair of inline
 *  ErrorNotices. The tier is NOT in config.json — it is an owner-only keystone
 *  leaf — so this reads/writes the dedicated `api.getDefaultApprovalMode` /
 *  `api.saveDefaultApprovalMode` pair keyed on its own `default-approval-mode`
 *  query, rather than the generic config endpoint YoloDurationCard uses.
 *
...
function DefaultApprovalModeCard() {
  ...
  const { data, isError: cfgError } = useQuery<DefaultApprovalModeData>({
    queryKey: DEFAULT_APPROVAL_MODE_QK,
    queryFn: api.getDefaultApprovalMode,
  })
  // `mode` is the server's EFFECTIVE tier: a non-persistable stored value is
  // already clamped to `normal` server-side, so the control shows what a NEW
  // session actually starts on rather than an aspirational value. `cfgError`
  // below is what keeps that honest when the read failed rather than the value
  // genuinely being `normal`.
  const configured = data?.mode
  ...
  // Keyed off the server's RAW `stored`, NOT off `configured`. The server clamps
  // before answering, so `configured` is always offerable and a cue comparing it
  // against the offerable set could never fire -- it would assert a protection that
  // does not run. `stored` is present only when the clamp actually changed
  // something, which is the state worth naming: the unset frame and the
  // stored-unpersistable frame were otherwise pixel identical, so a stored `trust`
  // showed Normal ticked with no hint it had been ignored.
  const overriddenValue = data?.stored
  const overridden = typeof overriddenValue === 'string' && overriddenValue.trim() !== ''
  // Optimistic + rollback, mirroring ComputerUsePanel's dedicated pair: the click
  // reflects immediately, a failed save rolls the query back to server truth and
  // raises the inline notice, and onSettled reconciles against the refreshed GET.
  const save = useMutation({
    mutationFn: (v: string) => api.saveDefaultApprovalMode(v),
    onMutate: async v => {
      setSaveError('')
      await qc.cancelQueries({ queryKey: DEFAULT_APPROVAL_MODE_QK })
      const prev = qc.getQueryData<DefaultApprovalModeData>(DEFAULT_APPROVAL_MODE_QK)
      if (prev) qc.setQueryData<DefaultApprovalModeData>(DEFAULT_APPROVAL_MODE_QK, { ...prev, mode: v })
      return { prev }
    },
    onError: (_err, _v, ctx) => {
      if (ctx?.prev) qc.setQueryData<DefaultApprovalModeData>(DEFAULT_APPROVAL_MODE_QK, ctx.prev)
      setSaveError(i18nT('pages.settings.securityPanel.default_approval_mode_save_failed'))
    },
    onSettled: () => qc.invalidateQueries({ queryKey: DEFAULT_APPROVAL_MODE_QK }),
  })
    ...
    <SettingsCard>
      ...
      {overridden && !cfgError && (
        <div className="text-[11px] text-warn mt-2 leading-relaxed">
          {i18nT('pages.settings.securityPanel.default_approval_mode_overridden', { value: String(overriddenValue) })}
        </div>
      )}
      ...
      {saveError && (
        <ErrorNotice variant="inline" className="mt-1.5" message={saveError} askAgent />
      )}
    </SettingsCard>
  )
}
