...
vi.mock('../../api/client', () => ({
  api: {
    ...
    tailnetStatus: vi.fn(),
    // This card's DEDICATED owner-only pair. `agent.default_approval_mode` is no
    // longer in config.json, so the tier is read/written through these.
    getDefaultApprovalMode: vi.fn(),
    saveDefaultApprovalMode: vi.fn(),
    // Still mocked: the sibling YoloDurationCard in the same approval section
    // reads config.json through kirocrewConfig on mount.
...
/** The offerable, persistable set the dedicated endpoint publishes alongside the
 *  effective mode. The card reads only `mode`, but the real payload carries both. */
const OFFERABLE = ['normal', 'trust_reads']

/** The GET payload as the dedicated endpoint returns it. The server has ALREADY
 *  clamped a non-persistable stored value to `normal`, so a `mode` outside
 *  OFFERABLE never comes from the real backend -- the tests below pass one only to
 *  exercise the card's own defence-in-depth clamp and its override cue. */
/**
 * The payload as the SERVER would answer it, clamp included.
 *
 * The backend clamps before answering, so it can never send an unpersistable
 * `mode`: an unpersistable stored value comes back as `mode: 'normal'` with the raw
 * value in `stored`. Modelling that here rather than seeding a raw `mode` is what
 * keeps these tests honest -- a fixture the server cannot produce would let the
 * override cue pass against a shape that never reaches the client.
 */
function payload(mode: string) {
  if (OFFERABLE.includes(mode)) return { mode, values: OFFERABLE }
  return { mode: 'normal', values: OFFERABLE, stored: mode }
}
...
async function renderCard(cfg: { mode: string; values: string[]; stored?: string }) {
  ;(api.getDefaultApprovalMode as ReturnType<typeof vi.fn>).mockResolvedValue(cfg)
  ;(api.saveDefaultApprovalMode as ReturnType<typeof vi.fn>).mockResolvedValue(cfg)
...
async function expectChecked(group: HTMLElement, key: string) {
  expect(api.getDefaultApprovalMode).toHaveBeenCalled()
...
describe('SecurityPanel -- default approval mode for new chats', () => {
  beforeEach(() => {
    ...
    // The sibling YoloDurationCard in this section reads config.json on mount;
    // keep it settled so it never errors in a test that is not about it.
  ...
  it('offers exactly the three declared tiers, in declared order', async () => {
    const { group } = await renderCard(payload('normal'))
  ...
  it('does NOT honour a stored trust, which is not persistable', async () => {
    const { group } = await renderCard(payload('trust'))
    await expectChecked(group, 'normal')
  })

  it('shows the reads tier under its existing label when trust_reads is stored', async () => {
    const { group } = await renderCard(payload('trust_reads'))
  ...
  it('falls back to Normal when the stored value is not an offerable tier', async () => {
    ...
    const { group } = await renderCard(payload('yolo'))
    await expectChecked(group, 'normal')
  })
  ...
  it('writes the DECLARED KEY, not the label spelling, when a tier is chosen', async () => {
    ...
    await waitFor(() => expect(api.saveDefaultApprovalMode).toHaveBeenCalledWith('trust_reads'))
    // The label spelling must never reach the wire.
    expect(api.saveDefaultApprovalMode).not.toHaveBeenCalledWith('reads')
  })

  it('writes nothing until the user actually moves it', async () => {
    const { group } = await renderCard(payload('normal'))
    // Mount alone must not write.
    expect(api.saveDefaultApprovalMode).not.toHaveBeenCalled()
  ...
  it('says so when the config read failed, rather than passing Normal off as stored', async () => {
    ;(api.getDefaultApprovalMode as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('nope'))
    ;(api.saveDefaultApprovalMode as ReturnType<typeof vi.fn>).mockResolvedValue({ mode: 'normal', values: OFFERABLE })
  ...
  it('surfaces a failed save', async () => {
    const { group } = await renderCard(payload('normal'))
    ;(api.saveDefaultApprovalMode as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('nope'))
    fireEvent.click(within(group).getByText(LABEL.trust_reads()))
    await waitFor(() =>
      expect(
