...
export const api = {
  ...
  projectPickerConfig: () => fetch('/api/project-picker/config').then(j) as Promise<{ folder_picker: boolean }>,
  pickProjectFolder: () => fetch('/api/pick-folder', { method: 'POST' }).then(j) as Promise<{ path: string | null; error?: string }>,
