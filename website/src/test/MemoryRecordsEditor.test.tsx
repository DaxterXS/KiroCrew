...
describe('memory record editing in both lineages', () => {
  ...
  it.each(['default', 'member-review'])('keeps recovery limits visible before applying a scoped %s forget', async store => {
    ...
    mount(store); await screen.findByText('Contact 0: old@example.com'); fireEvent.click(checkbox(0))
    fireEvent.click(screen.getByRole('button', { name: 'Forget selected memories: 1', exact: true }))
    const dialog = screen.getByRole('dialog', { name: 'Forget selected memories: 1', exact: true })
    ...
    expect(api.memoryEditApply).not.toHaveBeenCalled()
    fireEvent.click(within(dialog).getByRole('button', { name: 'Preview changes' }))
    const forget = await within(dialog).findByRole('button', { name: 'Forget selected memories: 1' })
