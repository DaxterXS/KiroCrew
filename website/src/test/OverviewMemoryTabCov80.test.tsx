...
describe('MemoryTab — lessons', () => {
  ...
  it('surfaces a post-delete refresh failure instead of swallowing it', async () => {
    // The delete succeeds on the backend; the list refresh (api.lessons) then
    // fails. That must surface -- otherwise the row looks un-deleted with no
    // explanation.
    api.deleteLesson.mockResolvedValue({ ok: true })
    render(<MemoryTab refreshTrigger={0} />)
    await screen.findByText('zzq-rule-beta')
    api.lessons.mockRejectedValueOnce(new Error('refresh GET failed'))
    const row = screen.getByText('zzq-rule-beta').closest('tr') as HTMLElement
    const del = Array.from(row.querySelectorAll('button'))
      .find((b) => /delete/i.test(b.textContent ?? '')) as HTMLButtonElement
    await userEvent.click(del)

    expect(await screen.findByRole('alert')).toHaveTextContent(/refresh GET failed/i)
  })
