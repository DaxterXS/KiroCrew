...
interface ChatState {
   ...
   *  utils/historyDeleteRefusal). Cleared on dismiss or on the next attempt. */
...
>(
  ...
  async (key, { getState, rejectWithValue }) => {
    try {
      ...
      return rejectWithValue({ key, title, code: parseErrorCode(typeof body === 'string' ? body : undefined) ?? '' })
    }
  },
)
