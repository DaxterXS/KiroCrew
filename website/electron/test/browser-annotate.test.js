...
test("overlay: valid JS that splices the walker's shared helpers (same refs, same names)", () => {
  ...
  // The walker never lists the overlay's own badges.
  assert.ok(WALKER_SOURCE.includes('if (el.id === "__kcAnnotateHost") continue;'));
  ...
  // pointer events included, since they fire before and independently of mouse events.
  ...
  for (const ev of ["mousedown", "mouseup", "pointerdown", "pointerup"]) {
    assert.ok(OVERLAY_SOURCE.includes(`document.addEventListener("${ev}", swallow, true)`), ev);
    assert.ok(OVERLAY_SOURCE.includes(`document.removeEventListener("${ev}", swallow, true)`), `${ev} teardown`);
  }
...
test("runAnnotateOp start: installs the overlay, then starts pick mode with the badge hint and numbering continuation", async () => {
  ...
  const res = await runAnnotateOp(wc, "start", { editHint: "Click to edit", seq: 2, idStart: 7, labels: { nope: 1 } });
  ...
  assert.deepStrictEqual(wc.calls, ["install", 'start:{"editHint":"Click to edit","seq":2,"idStart":7}']);
});
